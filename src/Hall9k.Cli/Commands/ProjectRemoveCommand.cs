...
public sealed class ProjectRemoveCommand : Hall9kAsyncCommand<ProjectRemoveCommand.Settings>
{
    ...
    /// these skips existed). The same predicate gates <c>--purge</c> (the acceptance criteria's own
    /// words: "the same refusal as archive applies"), checked fresh every time regardless of
    /// whether the project was archived moments ago or long before — a purge never assumes
    /// yesterday's archive check still holds.
    /// </summary>
