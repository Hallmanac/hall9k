...
/// (report it, create nothing) or not yet (report that too, create nothing, and let catch-up bring
/// the stream in — idea 202383dc, task 9408d525). An item with no record and no local task adopts
/// exactly as it always did: title to objective, body to context, criteria typed by hand.
/// </para>
/// </summary>
internal static class TaskRecordAdoption
{
    ...
    internal enum LocateOutcome
    {
        ...
        /// <summary>A record names this reference, and this store already holds a task at its own task id.</summary>
        ExistingLocal,
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (LedgerEntry entry in entries)
        {
            ...
            bool present = await session.LoadAsync<TaskListItem>(record.TaskId, cancellationToken) is not null;
            return new Locate(present ? LocateOutcome.ExistingLocal : LocateOutcome.StreamAbsent, record.TaskId);
        }
