...
public sealed class ProjectShowCommand : Hall9kAsyncCommand<ProjectShowCommand.Settings>
{
    ...
    protected override async Task<int> ExecuteAsync(Settings settings, CancellationToken cancellationToken)
    {
        ...
        bool notJoined = ProjectJoinStatus.NotJoined(project, owner?.RootFingerprint);
        OperatingSettings operatingSettings = (await PlatformConfigFile.TryReadOperatingSettingsAsync(cancellationToken)).Settings;

        AnsiConsole.Write(Registration(project, connection, owner, notJoined));
    ...
    private static Table Registration(ProjectDetails project, ConnectionDetails? connection, OwnerDetails? owner, bool notJoined)
    {
        ...
        if (notJoined)
        {
            ...
            // owner..."): the join deferred to an existing owner, never ran because the
            // repository was not reachable yet, an explicit --owner claim is still unverified, or
            // an --invite already pasted is still waiting on its own vouch. This local mirror
            // cannot tell an invite is actually needed from "this install owns this project and
            // simply has not joined it yet" (independent pre-PR review, cycle 1, conformance and
            // adversarial lenses, ProjectJoinStatus's own doc), so this names the plain join
            // command rather than presupposing --invite <token>. Named here, not folded into the
            // Owner row below, since it is the first thing a newcomer reading this pane after
            // registering actually needs to act on.
            table.AddRow("Joined", $"[yellow]not joined yet[/] [dim]— h9k project join "
                + $"{project.Name.EscapeMarkup()}[/]");
        }
        ...
        table.AddRow("Project key", project.ProjectKey is { } key
            ? $"[dim]{key.EscapeMarkup()}[/]"
            : $"[dim]not recorded yet — h9k project join {project.Name.EscapeMarkup()} reads it back once "
              + $"the ledger has one, or, for a ledger whose genesis predates it, h9k project assign-key "
              + $"{project.Name.EscapeMarkup()} mints it[/]");
    ...
    /// <summary>
    /// The compiled defaults (never removable, so always shown, dimmed) plus this project's
    ...
    /// pre-PR review, cycle 5, adversarial lens, medium). <paramref name="recorded"/> carries the
    /// same "was this ever recorded" distinction <see cref="ClaimGateRow"/>'s own doc explains
    /// (Decisions Log #161): a project nobody has ever run <c>h9k project set --take-policy</c> on
    /// and one an operator explicitly set back to <c>auto</c> are different facts, and a row that
    /// renders them identically is how the choice goes unnoticed (independent pre-PR review,
    /// cycle 6, conformance lens, low).
    /// </summary>
    internal static string TakePolicyRow(ProjectDetails project, bool recorded) =>
        project.TakePolicy == TakePolicy.Ask
            ...
            : $"[dim]auto ({OriginNote(recorded)}) — the holder's own node grants a cooperative take request "
              + $"on receipt when no run is live for the task, refuses otherwise; park it for a human "
              + $"instead: h9k project set {project.Name.EscapeMarkup()} --take-policy ask[/]";

    /// <summary>
    /// See <see cref="TakePolicyRow"/>'s own doc — the number of minutes that setting's own overdue
    /// wording actually means, and the identical "was this ever recorded" distinction
    /// <paramref name="recorded"/> carries: <c>TakeTimeoutMinutes</c> is an
    /// <c>Optional&lt;int?&gt;</c>, so an operator can explicitly reset it back to null the same
    /// way <c>TakePolicy</c> can be explicitly re-affirmed to <c>auto</c>, and a null read here
    /// must not silently collapse that into "nobody ever touched this" (class sweep off the
    /// TakePolicy fix, independent pre-PR review, cycle 6, conformance lens).
    /// </summary>
    internal static string TakeTimeoutRow(ProjectDetails project, bool recorded) =>
