...
public sealed class ReviewResolveCommand : Hall9kAsyncCommand<ReviewResolveCommand.Settings>
{
        ...
        GitHubReviewReplies replies, CancellationToken cancellationToken)
    {
        ...
        foreach (ReviewDisagreement disagreement in disagreements)
        {
            ...
            if (body.IsBlank())
            {
                // The thread id before the location, because a review-feedback lap's own park
                // routinely carries neither a location nor any words: a decline read off the
                // THREAD DISPOSITION triage alone drafts nothing, and naming "the review's own
                // body" there would point the operator at a review that does not exist (task: a
                // review-feedback follow-up never answers a human reviewer in the owner's name on
                // its own).
                string where = disagreement.ThreadId.IsNotBlank()
                ...
                throw new DomainConflictException(
                    ...
                    + "--post-reply \"<text>\", or resolve with --post-nothing.");
            }
