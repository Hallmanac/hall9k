...
/// shell, a payload shaped differently by a future Claude Code — every one exits 0 and lets the
...
public sealed class PullRequestReplyGuardCommand : Hall9kAsyncCommand<PullRequestReplyGuardCommand.Settings>
{
    ...
    /// <summary>
    /// The tool names whose <c>command</c> input this reads as a shell command, derived from the
    /// hook's own matcher so the registered tools and the answered tools cannot drift. Everything
    /// else answers false — see the class doc on failing open.
    /// </summary>
    private static readonly HashSet<string> GuardedShellTools = new(
        ClaudeSettingsFile.ReviewThreadReplyGuardMatcher.Split('|'), StringComparer.OrdinalIgnoreCase);
    ...
    internal static bool Denies(string? payload)
    {
        ...
        try
        {
            ...
            // with a matcher naming exactly these tools: a settings file an operator edited, or a
            // future matcher spelling, must not turn this into a guard over tools whose input it
            // cannot read. Both shells are here, not Bash alone — on this platform's own primary
            // host a dispatched session is handed a PowerShell tool beside its Bash one, and the
            // same gh api line runs in either, so a Bash-only answer was a refusal with the
            // ordinary Windows shell left open beside it (independent pre-PR review, cycle 1,
