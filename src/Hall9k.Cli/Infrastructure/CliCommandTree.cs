    ...
    public static void Configure(IConfigurator config, Func<string, string?> environmentVariable)
    {
        ...
        config.AddBranch("project", project =>
        {
            ...
            project.AddCommand<ProjectRemoveCommand>("remove")
                .WithDescription(
                    ...
                    + "stream, every task, run, idea, and epic stream it owns, and their projection "
                    + "documents) — cancellable until it fires (h9k project cancel-purge); linked tracker items, "
            ...
            project.AddCommand<ProjectPullCommand>("pull")
                .WithDescription(
                    ...
                    + "switch-on point, as it does a brand-new node's own bootstrap; an ordinary flush and "
                    + "a gap-fill are still held above it. Answers apply "
        ...
        config.AddBranch("config", operatingSettings =>
        {
            ...
            operatingSettings.AddCommand<ConfigSetCommand>("set")
                ...
                .WithExample("config", "set", "--effort-build", "xhigh", "--effort-review", "high")
                .WithExample("config", "set", "--effort-review-verify", "medium")
                .WithExample("config", "set", "--effort-courier", "default")
