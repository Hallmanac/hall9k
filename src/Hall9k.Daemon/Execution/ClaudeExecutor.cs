    ...
    ILogger<ClaudeExecutor> logger, IProcessManager processManager, IOptions<DaemonOptions> options) : IExecutor
{
    public async Task<SpawnedAgent> SpawnAsync(AgentSpawnRequest request, CancellationToken cancellationToken)
    {
        ...
        // HALL9K_DISPATCHED_RUN_ID (and HALL9K_DISPATCHED_TASK_ID, when the request names one) is
        // stamped here rather than by each of the eighteen AgentSpawnRequest builders (task: a
        // dispatched session cannot drive the project's own lifecycle): every daemon dispatch —
        // build, fix, review, verify, final pass, recovery, courier, and anything else this executor
        // ever spawns — is unattended by construction, so the one spawn site marks every one of them
        // rather than trusting each call site to remember. Inherited by every Bash-tool child the
        // session spawns, the same way HALL9K_DETACHED_SESSION already rides a headless launch's
        // environment.
        ...
        {
            environment.Add(new(DispatchedRunEnvironment.TaskIdVariable, taskId.ToString()));
        }
