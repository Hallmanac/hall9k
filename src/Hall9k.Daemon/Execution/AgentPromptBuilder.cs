...
public static class AgentPromptBuilder
{
    ...
    /// </para>
    /// <para>
    /// The section says out loud that the park does not depend on the marker, because it does
    /// not: <c>RunSupervisor</c> parks on a decline or a route of an observed human thread read
    /// off the <c>THREAD DISPOSITION:</c> triage alone, with a blank draft. Stating it is what
    /// keeps the instruction honest — a session told the park hangs on its own closing line would
    /// reasonably conclude the marker is the lever, when what the marker actually buys is its
    /// drafted words reaching the owner instead of a blank.
    /// </para>
    /// </summary>
    private static void AppendThreadDisputeRules(StringBuilder prompt, VoiceSkillName? voiceSkill)
    {
        ...
        AppendFragment(
            ...
            ("ProposedReplyMarker", ReviewResultParser.ProposedReplyMarker));
        AppendFragment(
            prompt, file, "platform-parks",
            ("DisputeMarker", DisputeMarker),
            ("DispositionTagKey", ReviewResultParser.DispositionTagKey));
