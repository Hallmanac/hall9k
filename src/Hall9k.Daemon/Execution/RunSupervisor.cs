    ...
    ILogger<RunSupervisor> logger)
{
    ...
    /// </summary>
    /// <param name="isDeliberateHeadlessStart">
    /// True only for a run <see cref="AdoptDeliberateHeadlessStartsAsync"/> is picking up (task: a
    /// do-now session launched by h9k task start is caught within seconds) — every other caller
    /// leaves this at its default false, which keeps the daemon's own ordinary dispatch path
    /// (<see cref="RunLauncher"/>, and every other <see cref="StartMonitoring"/> caller in this
    /// file) byte-for-byte unchanged: <see cref="MonitorAsync"/> only branches on it at the two
    /// points below where the two shapes actually diverge.
    /// </param>
    public void StartMonitoring(
        Guid runId, string runDirectory, Guid taskId, int processId, DateTimeOffset processStartedAt,
        CancellationToken cancellationToken, bool isDeliberateHeadlessStart = false)
    {
    ...
    public async Task<OrphanAdoption> AdoptOrphansAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (RunDetails run in candidates)
        {
            ...
            if (run.State == RunState.Verifying && run.PendingBuildSessionErrorRetry)
            {
                // AgentSessionCompleted moved this run to Verifying the instant the errored
                // build session's own completion was recorded — before the retry's resumed
                // spawn ever happened, let alone landed — so reading Verifying here as "the
                // work is done" would hand the review loop a build the agent never actually
                // resumed (adversarial pre-PR review, cycle 1). Finish what CompleteRunAsync
                // started instead of adopting it into the pre-PR pipeline. No second backoff:
                // the elapsed downtime already stands in for it.
                logger.LogInformation(
                ...
                switch (retried.Outcome)
                {
                    case BuildSessionRetryOutcome.Resumed:
                        StartMonitoring(
                            run.Id, run.RunDirectory, run.TaskId, retried.ProcessId, retried.ProcessStartedAt,
                            cancellationToken);
                        await RefreshAdoptedLeaseAsync(run, cancellationToken);
                        adopted++;
                        break;
                    case BuildSessionRetryOutcome.ClaimMovedOn:
                        // Already retired with RunSuperseded — not counted either way, the
                        // same convention RetireStaleAdoptionCandidateAsync's callers use above.
                        break;
                    case BuildSessionRetryOutcome.SpawnFailed:
                    default:
                        await FailRunAsync(run.Id, run.TaskId, RunDetails.ErrorResultRetryCouldNotResume, cancellationToken);
                        failed++;
                        break;
                }
        ...
        try
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                if ((!alive && sawAnyResult) || endedAfterResultGrace)
                {
                    ...
                    // so the platform decides for itself whether to deliver or flag it, instead of
                    // CompleteRunAsync's error/budget/pr-review/verify-review-PR machinery, none of
                    // which this unattended shape was ever meant to enter.
                    if (isDeliberateHeadlessStart)
                    {
                        await HandleDeliberateHeadlessStartExitAsync(runId, taskId, runDirectory, result, cancellationToken);
                        return;
                    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (willRetryBuildSession)
        {
            ...
            switch (retried.Outcome)
            {
                ...
                case BuildSessionRetryOutcome.SpawnFailed:
                default:
                    // The retry was already recorded as spent above; only the spawn itself
                    // failed (missing task/project docs, or the resumed spawn throwing) — a
                    // distinct reason from the plain agent error, so a reader can tell "the
                    // agent errored twice" apart from "the machinery never gave it a second
                    // chance" (RunDetails.ErrorResultRetryCouldNotResume's own doc).
                    await FailRunAsync(runId, taskId, RunDetails.ErrorResultRetryCouldNotResume, cancellationToken);
                    return null;
            }
        }
    ...
    private static readonly string[] AdoptableDeliberateHeadlessStartStates =
        [RunState.Dispatched.Value, RunState.Running.Value];

    /// <summary>
    /// The running-daemon counterpart of <see cref="AdoptOrphansAsync"/> for a shape that sweep
    /// never sees at all (task: a do-now session launched by h9k task start is caught within
    /// seconds): a <c>h9k task start</c> claim's own run carries the same ceiling-exempt
    /// <see cref="Guid.Empty"/> NodeId an operator's own interactive claim does, so it is
    /// deliberately invisible to every ordinary <c>NodeId == nodeId</c> query — the whole point of
    /// that sentinel is "a human owns this, not a daemon". But unlike an interactive claim, nothing
    /// actually attends this one: the CLI process that launched it exits the moment the detached
    /// agent is confirmed alive (<c>HeadlessLaunch.SpawnDetached</c>'s own doc), so its session can
    /// finish and exit with no daemon and no human ever noticing — the origin incident this run
    /// closes (task ef2fefe5: a clean, one-commit session sat undelivered for an hour). Scoped to
    /// <see cref="RunDetails.DispatchingNodeId"/> rather than <see cref="RunDetails.NodeId"/>, the
    /// identical reasoning <see cref="SentinelPrReviewCandidatesAsync"/> already applies to its own
    /// sentinel widening: the sentinel itself carries no node identity, so without this a
    /// multi-node install sharing one database would let every node's daemon race to adopt the
    /// same claim.
    /// <para>
    /// Called every dispatch cycle, the same cadence <see cref="ResumeStrandedPipelinesAsync"/>
    /// already runs at (<c>DispatchLoop</c>'s own per-cycle loop) — not only at startup, since a
    /// session <c>h9k task start</c> just launched can finish and exit while this daemon is
    /// already running, and nothing else would ever notice. <see cref="RunDetails.ExitedUnattendedReason"/>
    /// being null is what keeps this from re-adopting (and re-checking the worktree of) a run this
    /// sweep already flagged on an earlier cycle — once one of the three levers the flag names
    /// moves <see cref="RunState"/> off Dispatched/Running, the state filter below stops matching
        ...
        Guid runId, Guid taskId, string runDirectory, AgentResult? result, CancellationToken cancellationToken)
    {
        ...
        {
            // Already moved on — delivered, released, handed back, or re-entered by a human
            // between this run's own dispatch and this moment. Nothing here is this sweep's to
            // decide any more.
            return;
        }
        ...
        if (check.Observed && check.FailureReason is null && onClaimBranch)
        {
            ...
            {
                logger.LogInformation(
                    "Task {TaskId}: lost the generation race delivering run {RunId} automatically — a newer "
                    + "claim committed first",
                    taskId, runId);
                return;
            }
    ...
    /// <summary>
    /// What the build session's error-result retry attempt produced, once its guard-and-spawn
    /// leg (<see cref="ResumeBuildSessionErrorRetryAsync"/>) has run.
    /// </summary>
    private enum BuildSessionRetryOutcome
    {
        /// <summary>The resumed session spawned; the caller keeps tailing it under the new process identity.</summary>
        Resumed,
        /// <summary>
        /// The task's claim moved on before the spawn (an abandon, a lease-expiry
        /// requeue-and-reclaim, or any other release) — nothing here was left to resume, and
        /// the run has already been retired with <see cref="RunSuperseded"/> rather than
        /// failed, since the agent's own error is not what ended this run's lifecycle.
        /// </summary>
        ClaimMovedOn,
    ...
    /// session's own completion facts sat unsaved in memory across a real-time delay.
    /// </summary>
    ...
    /// <summary>
    /// The guard-and-spawn core of the build session's error-result retry, apart from the
    /// backoff wait itself: <see cref="AdoptOrphansAsync"/> calls this directly, with no second
    /// backoff, for a run whose retry was durably recorded (<see cref="RunDetails.PendingBuildSessionErrorRetry"/>)
    /// but never actually resumed before the daemon restarted — the elapsed downtime already
    /// stands in for the wait, and adoption must not block startup behind another full backoff.
    /// </summary>
    private async Task<(BuildSessionRetryOutcome Outcome, int ProcessId, DateTimeOffset ProcessStartedAt)> ResumeBuildSessionErrorRetryAsync(
        Guid runId, Guid taskId, CancellationToken cancellationToken)
    {
        await using IDocumentSession session = store.LightweightSession();
        ...
        {
            return (BuildSessionRetryOutcome.SpawnFailed, 0, default);
        }

        if (task.State != TaskState.Claimed || task.CurrentRunId != runId)
        {
            // The claim moved on during the backoff wait — h9k task abandon, a lease-expiry
            // requeue-and-reclaim, or any other release — the same guard
            // TokenBudgetRetryEngine.RetryOneAsync already applies before its own resume spawn
            // (adversarial pre-PR review, cycle 1): there is nothing here left to resume, and
            // spawning anyway would double-book a worktree this task no longer owns. Retiring
            // the run explicitly, rather than returning and leaving it live at Verifying with
            // no monitor, mirrors every other generation-fence rejection in this file
            // (ParkedOnThreadDisputeAsync, RetireStaleAdoptionCandidateAsync) — a bare return
            // would otherwise pin a NodeLoad slot and an AdoptOrphansAsync candidate forever,
            // since Verifying with the retry already spent reads as "the work is done" to
            // every reader that does not separately check PendingBuildSessionErrorRetry.
            session.Events.Append(runId, new RunSuperseded(runId, task.LeaseGeneration, DateTimeOffset.UtcNow));
            await session.SaveChangesAsync(cancellationToken);
            logger.LogInformation(
                "Run {RunId}: retired as superseded — the error-result retry found it was no longer task {TaskId}'s current generation",
                runId, taskId);
            return (BuildSessionRetryOutcome.ClaimMovedOn, 0, default);
        }
        ...
        Guid runId, Guid taskId, AgentResult result, CancellationToken cancellationToken)
    {
        ...
        IReadOnlyList<ReviewDisagreement> humanThreadDrafts = HumanThreadReplyDrafts(task, result.Summary);
        ...
        logger.LogWarning(
            isChangesRequestedDisagreement
                ...
                : humanThreadDrafts.Count > 0
                    ? "Run {RunId}: drafted a reply to a human reviewer's thread and posted nothing — parked for the human. {Reason}"
                    : isRebaseDispute
    ...
    /// </summary>
    private static IReadOnlyList<ReviewDisagreement> HumanThreadReplyDrafts(TaskDetails task, string? summary)
    {
        ...
        foreach (ReviewDisagreement disagreement in ReviewResultParser.ParseDisagreements(summary))
        {
            ...
            // different thread rather than the same one spelled differently — the same comparison
            // ReviewResolveCommand's own vet of this id uses.
            ReviewThreadReference? observed = task.HumanReviewThreads.Find(thread =>
                string.Equals(thread.ThreadId, disagreement.ThreadId, StringComparison.Ordinal));
            if (observed is null)
            {
                continue;
            }
    ...
    {
        string lead = drafts.Count == 1
            ? "A review-feedback follow-up answered a thread a person opened and posted nothing: "
            : $"A review-feedback follow-up answered {drafts.Count} threads people opened and posted nothing: ";
        ...
        return lead
            ...
            + "(--post-reply \"<text>\"), or post nothing (--post-nothing). Nothing has been pushed, and the "
            + "reviewer has heard nothing.";
