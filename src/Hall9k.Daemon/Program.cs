...
// perform, which IConfiguration's own merged view of env, the config file, and appsettings.json
// cannot express: whether a level's own answer came from max-concurrent-task-runs directly or from
// converting max-concurrent-agent-sessions has to be decided per level, not from one flattened key.
...
builder.Services.AddSingleton<LaunchHoldEngine>();
...
builder.Services.AddSingleton<CourierEngine>();
...
builder.Services.AddHostedService<CourierSweepLoop>();
builder.Services.AddHostedService<RunSkillSweepLoop>();
builder.Services.AddHostedService<LocalLaunchSweepLoop>();
