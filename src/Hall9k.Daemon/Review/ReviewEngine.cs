    ...
    CloseoutEngine closeout)
{
    ...
    /// <para>
    /// <paramref name="gitProcessRunner"/> is what production actually binds to this deadline
    /// (<c>ExternalProcess.RunnerWithDeadline(GitDeadline)</c>, wired in <c>Program.cs</c>) for the
    ...
    /// <summary>
    /// How many pre-final-pass rebase-recovery sessions one run may have dispatched in a row
    /// without ever reaching a clean rebase (task: a run rebases its branch onto the current base
    /// branch) before <see cref="DispatchRebaseRecoverySessionAsync"/> parks instead of spawning
    /// another one — see <see cref="RunAggregate.RebaseRecoveryRounds"/>'s own doc for the loop
    /// this bounds. Deliberately a fixed, uninherited constant rather than a task/project/node
    /// override chain like the review-cycle caps: this is a narrow, mechanical safety bound against
    /// a session that never actually resolves the conflict, not a knob a team is expected to tune.
    /// </summary>
    private const int MaxRebaseRecoveryRounds = 3;

    // internal, not private: the stack-assessment methods below that return this are internal too,
    // exercised directly by tests over a fake ProcessRunner and a scripted assessment response
    // rather than through the real-git-dependent StackedParentWatch/plain-rebase preconditions
    // that gate their own only production call sites (task: a stacked checkpoint that would park
    // for a human on a git shape first dispatches a read-only assessment run — "no git integration
    // tests").
    internal enum RebaseGateOutcome
    {
            ...
            switch (run.ReviewPhase)
            {
                case ReviewPhase.None:
                    if (run.ReviewStageComposition == ReviewStageComposition.None)
                    {
                        ...
                        // eventually read when it lands, PLAN.md Decisions Log #129.
                        //
                        // The rebase still runs first (task: a run rebases its branch onto the
                        // current base branch, independent pre-PR review, cycle 1, both lenses):
                        // waiving the review guarantee is not the same as waiving the mergeable-
                        // on-arrival one, and this composition's PullRequestOpener push is exactly
                        // as exposed to a moved base as any other's. A conflict here dispatches the
                        // recovery session and loops; its own completion lands on
                        // ReviewPhase.Settling, whose own composition-none check just below settles
                        // it the same way this branch would have.
                        bool noReviewRebaseLoopAgain = false;
                        switch (await EnsureRebasedBeforeFinalPassAsync(context, run, cancellationToken))
                        {
                            ...
                            case RebaseGateOutcome.LoopAgain:
                                noReviewRebaseLoopAgain = true;
                                break;
                        }

                        if (noReviewRebaseLoopAgain)
                        {
                            break;
                        }
                    ...
                    {
                        // A checkpoint conflict either parks (Stop) or, since this task, can also
                        // earn a dedicated fix session for a Replay verdict's own retry that still
                        // conflicts (LoopAgain) — the session dispatched by
                        // ActOnStackAssessmentAsync's own Replay branch, whose completion is what
                        // asks a caller to re-enter the loop, exactly like every other LoopAgain
                        // this switch already handles (the composition-none branch above, and the
                        // Settling branch below): break out to the top of the outer while loop
                        // rather than returning false, which would read as a park or failure this
                        // run has not actually reached — the dispatched session is still running.
                        bool stackedCheckpointLoopAgain = false;
                        switch (await RebaseOntoStackedParentAsync(
                            context, StackedCheckpoint.BeforeFirstReviewCycle, cancellationToken))
                        {
                            case RebaseGateOutcome.Stop:
                                return false;
                            case RebaseGateOutcome.LoopAgain:
                                stackedCheckpointLoopAgain = true;
                                break;
                        }

                        if (stackedCheckpointLoopAgain)
                        {
                            break;
                        }
                ...
                case ReviewPhase.Settling:
                    ...
                    // Run unconditionally on every Settling entry (task: a run rebases its branch
                    // onto the current base branch, independent pre-PR review, cycle 1, both
                    // lenses) — including the ordinary "nothing owed" settle path below, where a
                    // clean Discovery-cycle-1 convergence is the single most common way a run ends
                    // and, with the longest window since dispatch, the one most likely to have a
                    // moved base behind it. Gating this on needsFullGateBeforeSettling,
                    // verifyCommandsFingerprintChanged or HumanEndedTheLoop the way this once read
                    // meant that path opened its pull request having never fetched or rebased at
                    // all — exactly the staleness this task exists to catch before the remote ever
                    // sees it. Unconditional is affordable per Brian's 2026-09-04 ruling this
                    // method's own doc already rests on: a no-op or a clean git apply costs a
                    // fetch, not a session or an extra reviewer pass, so paying it on every settle
                    // (rather than only the mandatory-gate paths) does not reopen that ruling.
                    ...
                    // above like any other (independent pre-PR review, cycle 1, both lenses): its
                    // own recovery session's completion always records Settling, never back to
                    // ReviewPhase.None, so this is the one other place this composition's run can
                    // land. Short-circuited here, before the ordinary settle-reason logic below
                    // (which assumes at least one track ever opened) the same way the Reverify
                    // branch's own identical composition-none check further down already does —
                    // reusing that logic here would call DeriveSettlement/DeriveResidualTally over
                    // a run with no tracks and no completed passes, and could record the narrower
                    // Clean claim SettleWithNoReviewAsync's own doc says this composition can never
                    // honestly make.
                    if (run.ReviewStageComposition == ReviewStageComposition.None)
                    {
                        switch (await EnsureGateCoversHeadBeforeSettlingWithNoReviewAsync(context, run, cancellationToken))
                        ...
                        || run.PreFinalPassRebaseAwaitingReview)
                    {
                        ...
                        if (!gateAlreadyRanFullOverCurrentHead)
                        {
                            ...
                            //
                            // A failure here is repair-eligible whenever the most recent recorded
                            // rebase was real (task: a pre-final-pass rebase that applies cleanly
                            // but breaks the mandatory gate gets a repair lap inside the same run
                            // instead of failing it) — checked once, off the aggregate alone, no git
                            // or gate output involved, so it reads the same fact whether this is the
                            // very first failure or a later one after an earlier repair round.
                        ...
                        // a pre-final-pass rebase that left PreFinalPassRebaseAwaitingGate set but NOT
                        // PreFinalPassRebaseAwaitingReview — a clean git apply, never a recovery —
                        // never moved anything a reviewer would read differently: the rebase replays
                        // this cycle's own already-reviewed commits onto a new base rather than
                        // introducing any a fresh context has not read, so the diff already converged
                        // clean under this very cycle's own fresh-context passes (independent pre-PR
                        // review, cycle 3, adversarial lens — dispatching a whole extra FinalFullPass
                        // here re-reads a byte-identical diff a second time and spends it against
                        // MaxFinalFullPassRounds for nothing). A recovered rebase IS a reviewer-relevant
                        // change (independent pre-PR review, cycle 1, conformance lens: a recovery
                        // session resolves a real conflict with judgment, not a mechanical apply), so
                        // MaySettleReason's own NothingOwed clause is gated on
                        // PreFinalPassRebaseAwaitingReview being clear — its Human clause deliberately
                        // is not (AGENTS.md documents a human's own merge-ready resolve as needing only
                        // this mandatory gate, never a fresh automatic review). The gate above is what
                        // none of the OTHER cases ever covered (a rebase changes what the tree builds
                        // and runs against even when the diff it carries does not), and it has now
                        // actually run, so the run may settle. This is term-for-term MaySettleReason
                        // (the Reverify branch's own settle check below calls it by name) rather than a
                        // second copy of its logic, so the two branches cannot drift apart the next
                        // time either grows a condition (independent pre-PR review, cycle 5,
                        // conformance lens). Because MaySettleReason's own human clause takes this
                        // short-circuit unconditionally, a human's merge-ready resolve now settles
                        // straight from here without ever reaching FinalFullPassCapReached below — see
                        // RunAggregate.Apply(ReviewParkResolved)'s own note on what that means for its
                        // FinalFullPassRounds reset.
                    ...
                    // ungated human resolution, a pre-final-pass rebase still awaiting its own gate
                    // (RunAggregate.PreFinalPassRebaseAwaitingGate), or one still awaiting its own
                    // review (RunAggregate.PreFinalPassRebaseAwaitingReview) held — which guarantees
                    // MaySettleReason's own NothingOwed clause its extra conjunct too, so
                    // MaySettleReason is guaranteed non-null here rather than a fact this call site
                    // has to re-establish on its own.
                ...
                case ReviewPhase.AwaitingSettlingGateRepair:
                    if (!await AwaitSettlingGateRepairSessionAsync(context, run, cancellationToken))
                    {
                        return false;
                    }
                ...
                case ReviewPhase.SettlingGateRepairNeeded:
                    if (!await DispatchSettlingGateRepairSessionAsync(
                        context, run, run.LastSettlingGateRepairOutput ?? "(gate output unavailable)",
                        run.PendingSettlingGateRepairGuidance, enforceCap: false, cancellationToken))
                    {
                        return false;
                    }
                ...
                case ReviewPhase.Reverify:
                    ...
                    // composition-none branch above, PLAN.md Decisions Log #129.
        ...
        ReviewContext context, StackedCheckpoint checkpoint, CancellationToken cancellationToken)
    {
        ...
        {
            // A spent rebase budget is a resource cap, not a git shape an assessment could ever
            // resolve differently — the remaining three Park reasons StackedCheckpointPolicy
            // hands back (ParentMergedElsewhere, ParentDead, ParentUnresolvable) are exactly the
            // git shapes this task names, so only those earn the one read-only assessment this
            // run gets before parking (task: a stacked checkpoint that would park for a human on
            // a git shape first dispatches a read-only assessment run).
            bool isGitShapePark = observation.Verdict is StackedParentVerdict.ParentMergedElsewhere
                or StackedParentVerdict.ParentDead or StackedParentVerdict.ParentUnresolvable;
            if (!isGitShapePark)
            {
                await ParkAsync(
            ...
            StackAssessmentParkKind parkKind = observation.Verdict == StackedParentVerdict.ParentMergedElsewhere
                ? StackAssessmentParkKind.ParentMergedElsewhere
                : StackAssessmentParkKind.UndecidableBoundary;
        ...
        // attempt every stacked checkpoint has always made, unchanged by this task, so the tests
        // already written against it keep exercising real git — only the assessment-driven retry
        // below (ActOnStackAssessmentAsync) uses the injected, fake-git-testable one.
        return await ReplayCheckpointAsync(
            context, checkpoint, parentBranch, verdict.UpstreamCommit, verdict.OntoCommit, verdict.Reason,
            ExternalProcess.RunnerWithDeadline(GitDeadline), cancellationToken);
    }

    /// <summary>
    /// Attempts a stacked checkpoint's mechanical replay — <c>git rebase --onto</c> from
    /// <paramref name="upstreamCommit"/> onto <paramref name="ontoCommit"/> — and, on a conflict,
    /// dispatches this run's one read-only stack assessment before parking (task: a stacked
    ...
    internal async Task<RebaseGateOutcome> ReplayCheckpointAsync(
        ReviewContext context, StackedCheckpoint checkpoint, string parentBranch, string upstreamCommit,
        string ontoCommit, string reason, ProcessRunner git, CancellationToken cancellationToken)
    {
        string worktreePath = context.Run.WorktreePath;
        string? preRebaseHead = null;
        await using (IAsyncDisposable repositoryLock =
            await worktrees.AcquireRepositoryLockAsync(context.Project.RepositoryPath, cancellationToken))
        {
            ...
            try
            {
                ...
                {
                    logger.LogWarning(
                        "Run {RunId}: the commit {OntoCommit} named for the checkpoint {Checkpoint} replay is not in this repository's object store, so it cannot run — proceeding unrebased; closeout's own replay still covers a moved parent once this branch is pushed",
                        context.RunId, ShortSha(ontoCommit), checkpoint.Value);
                    return RebaseGateOutcome.Proceed;
                }
                ...
                ProcessResult replay = await git(
                    "git", ["rebase", "--onto", ontoCommit, upstreamCommit], worktreePath, cancellationToken);
            ...
            {
                if (await RebaseActuallyLandedAsync(worktreePath, context.Run.Branch, ontoCommit, cancellationToken))
                {
                    await RecordStackedCheckpointAsync(
                        context, parentBranch, checkpoint,
                        new StackedCheckpointVerdict(StackedCheckpointAction.Replay, upstreamCommit, ontoCommit, reason),
        ...
        string conflictReason =
            ...
            + $"{upstreamCommit}`), then resolve with h9k review resolve — or hand the conflict to a fix "
            + "session with h9k review resolve --needs-fixes \"<guidance>\".";

        return await AssessOrParkAsync(
            context, StackAssessmentParkKind.ReplayConflict, context.Run.Branch, parentBranch, upstreamCommit,
            ontoCommit, conflictReason,
            (assessed, ct) => ActOnStackAssessmentAsync(context, checkpoint, parentBranch, conflictReason, assessed, ct),
            cancellationToken);
    }
    ...
    internal async Task<RebaseGateOutcome> ActOnStackAssessmentAsync(
        ...
        StackAssessmentVerdict assessment, CancellationToken cancellationToken)
    {
        if (assessment.Kind == StackAssessmentVerdictKind.Aligned)
        {
            logger.LogInformation(
                "Run {RunId}: the stack assessment found the checkpoint {Checkpoint} already aligned — {Evidence}",
                context.RunId, checkpoint.Value, assessment.Evidence);
            return RebaseGateOutcome.Proceed;
        }
        ...
        await ParkAsync(
            context.RunId, context.TaskId,
            $"{parkText} The read-only stack assessment dispatched before this park could not resolve it "
    ...
    /// <summary>
    /// The one entry point every git-shape park goes through (task: a stacked checkpoint that
    /// would park for a human on a git shape first dispatches a read-only assessment run):
    /// dispatches this run's one read-only stack assessment if it has not already spent it, and
    /// either hands the fresh verdict to <paramref name="actOnVerdict"/> or, if this run already
    /// spent its one assessment, defers to <paramref name="onAssessmentAlreadySpent"/> — the
    /// never-loop-twice-on-the-same-park rule, enforced by a fresh read of
    /// <see cref="RunDetails.HasDispatchedStackAssessment"/> rather than a counter passed down the
    /// call stack, so every caller (a checkpoint's own park, a checkpoint's replay conflict, the
    /// pre-final-pass rebase's conflict) shares the identical one-shot guarantee without having to
    /// reason about how many times it has already recursed.
    /// </summary>
        ...
        IReadOnlyList<string>? additionalCandidateBranches = null)
    {
        ...
        await using (IQuerySession query = store.QuerySession())
        {
            run = await query.LoadAsync<RunDetails>(context.RunId, cancellationToken);
        }

        if (run is not null && run.HasDispatchedStackAssessment)
        {
            if (onAssessmentAlreadySpent is not null)
            {
                return await onAssessmentAlreadySpent(run, cancellationToken);
            }

            await ParkAsync(
                context.RunId, context.TaskId, BothDiagnosesParkReason(run, parkText),
                cancellationToken: cancellationToken);
            return RebaseGateOutcome.Stop;
        }
    ...
    /// <summary>
    /// What a second git-shape park on the same run reads, once this run has already spent its one
    /// assessment. <see cref="RunDetails.LastStackAssessmentVerdict"/> can still be blank here — a
    /// daemon restart between <see cref="Events.StackAssessmentDispatched"/> and
    /// <see cref="Events.StackAssessmentCompleted"/> leaves exactly that gap on the stream — and
    /// this never guesses at what the missing verdict might have been.
    /// </summary>
    internal static string BothDiagnosesParkReason(RunDetails run, string newParkText) =>
        run.LastStackAssessmentVerdict.IsNotBlank()
            ? $"{newParkText} This run already dispatched its one read-only stack assessment earlier and reached "
              + $"'{run.LastStackAssessmentVerdict}': {run.LastStackAssessmentEvidence} That assessment is not "
              + "spent twice — a human decides this with both diagnoses in hand rather than a second one being dispatched."
            : $"{newParkText} This run already dispatched its one read-only stack assessment earlier, but it left "
              + "no recorded verdict — most likely an interruption before it could finish. That assessment is not "
              + "spent twice regardless; resolve this by hand rather than waiting on a second one.";

    /// <summary>
    /// Spawns this run's one read-only stack assessment session, waits for it (bounded twice over
    /// by <see cref="DaemonOptions.StackAssessmentMaxTurns"/> and
    /// <see cref="DaemonOptions.StackAssessmentTimeout"/>, the identical "narrow, mid-run,
    /// synchronous spawn-and-wait" shape <c>VerificationRunner.RecoverUncommittedWorkOrExplainAsync</c>
    /// already uses), parses its trailer, and records the outcome — never itself decides what the
    /// caller does with the verdict. A spawn failure, an abandoned wait, or an unparseable trailer
    /// all resolve to <see cref="StackAssessmentVerdictKind.Undecidable"/>, the same "a missing or
    /// malformed trailer is undecidable" contract <see cref="StackAssessmentResultParser"/> already
    /// keeps.
    /// </summary>
        ...
        IReadOnlyList<string>? additionalCandidateBranches = null)
    {
        ...
        string pullRequestBase = string.Empty;
        if (context.Task.PullRequestUrl.IsNotBlank())
        {
            try
            {
                ...
                pullRequestBase = facts.BaseRefName;
            }
            catch (Exception exception) when (exception is not OperationCanceledException)
            {
                logger.LogWarning(
                    exception,
                    "Run {RunId}: could not read the pull request's own GitHub facts before dispatching the stack assessment — the session can still read them itself",
                    context.RunId);
            }
        }
        ...
        // decision before the wait" discipline VerificationRunner's own recovery attempt follows,
        // and what makes this run's one assessment genuinely one-shot even across a restart.
        await using (IDocumentSession dispatchSession = store.LightweightSession())
        {
            RunAggregate dispatchRun = await LoadRunAsync(context.RunId, cancellationToken);
            if (!await GenerationFence.AllowsAsync(
                dispatchSession, logger, context.TaskId, context.RunId, dispatchRun.LeaseGeneration,
                nameof(StackAssessmentDispatched), cancellationToken))
            {
                return StackAssessmentVerdict.Undecidable(
                    "this run's own lease generation had already moved on by the time the stack assessment would have dispatched");
            }

            dispatchSession.Events.Append(context.RunId, new StackAssessmentDispatched(
                context.RunId, parkKind, childBranch, parentBranch, recordedForkPoint, attemptedOntoCommit,
                sessionId, model, DateTimeOffset.UtcNow, sessionName));
        ...
        try
        {
            ...
            {
                logger.LogWarning(exception, "Run {RunId}: the stack assessment session could not be spawned", context.RunId);
            ...
            unfinished = agent;
            // Keyed by the session ARTIFACT name (SessionRoleName.StackAssessment), not sessionName:
            // the stream file's own name never carries the task-id prefix SessionName does — the
            // identical distinction DispatchRebaseRecoverySessionAsync's own artifactName/sessionName
            // pair keeps, and the one a same-session mistake here would silently read as "died
            // without a result" (independent self-review: File.Exists came back false against the
            // prefixed name every production and scripted spawn actually writes to).
            string runDirectory = RunPaths.ResolveCurrentDirectory(context.Run.RunDirectory);
            string streamFile = RunPaths.SessionStreamFile(runDirectory, SessionRoleName.StackAssessment);

            using CancellationTokenSource timeoutSource = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);
            timeoutSource.CancelAfter(_options.StackAssessmentTimeout);
            try
            {
            ...
            catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
            {
                ...
                unfinished = null;
                logger.LogWarning(
                    "Run {RunId}: the stack assessment session exceeded its {Timeout} bound and was terminated",
                    context.RunId, _options.StackAssessmentTimeout);
                result = null;
            }
        }
        catch (Exception exception)
        {
            if (unfinished is { } orphan)
            {
                ...
                {
                    logger.LogWarning(
                        terminateException, "Run {RunId}: could not terminate the abandoned stack assessment session (pid {ProcessId})",
                        context.RunId, orphan.ProcessId);
                }
            }
            ...
            logger.LogWarning(exception, "Run {RunId}: the stack assessment session failed before it could finish", context.RunId);
            return await RecordStackAssessmentCompletedAsync(
                context, parkKind, parentBranch, pullRequestBase, model,
        ...
        {
            logger.LogWarning(
                "Run {RunId}: the stack assessment session ended in error: {Summary}", context.RunId, result.Summary ?? "(no message)");
        }
    ...
    /// <summary>
    /// Records a stack assessment's own outcome (task: a stacked checkpoint that would park for a
    /// human on a git shape first dispatches a read-only assessment run): the verdict itself, and
    /// — for aligned or replay only — this run's own recorded fork point and base branch, moved to
    /// what the assessment actually observed, so a later checkpoint on this same run reads the
        ...
        IReadOnlyList<string>? additionalCandidateBranches = null)
    {
            ...
            && !await CommitResolvesAsync(context.Run.WorktreePath, verdict.BoundaryCommit, cancellationToken))
        {
            verdict = StackAssessmentVerdict.Undecidable(
                $"the assessment declared {verdict.Kind} with a BOUNDARY commit ({verdict.BoundaryCommit}) this "
                + $"repository cannot resolve, so it is not trusted as this run's own fork point. Its own evidence: "
                + $"{verdict.Evidence}");
        }
        ...
        string runDirectory = RunPaths.ResolveCurrentDirectory(context.Run.RunDirectory);
        Exception? evidenceWriteFailure = await RunPaths.AppendDisputePositionAsync(
            RunPaths.StackAssessmentEvidenceFile(runDirectory), verdict.Evidence, "Stack assessment evidence", cancellationToken);
        if (evidenceWriteFailure is not null)
        {
            logger.LogWarning(
                evidenceWriteFailure, "Run {RunId}: could not write the stack assessment's own evidence to {FilePath}",
                context.RunId, RunPaths.StackAssessmentEvidenceFile(runDirectory));
        }
    ...
    private async Task<bool> CommitResolvesAsync(string worktreePath, string commit, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ProcessResult result = await processRunner(
                "git", ["rev-parse", "--verify", "--quiet", $"{commit}^{{commit}}"], worktreePath, cancellationToken);
    ...
    /// <summary>
    /// Confirms which named branch (if any) <paramref name="ontoCommit"/> is the current tip of,
    /// checked only against the candidates the daemon already knows about — never derived by
        ...
        string worktreePath, string ontoCommit, IReadOnlyList<string> candidateBranches, CancellationToken cancellationToken)
    {
        ...
        foreach (string candidate in candidateBranches.Where(branch => branch.IsNotBlank()).Distinct())
        {
            ...
            {
                logger.LogWarning(
                    exception, "Could not resolve the candidate branch {Candidate} while confirming a stack assessment's onto commit", candidate);
            }
        }
    ...
    /// own boundary and onto commit, and undecidable falls back to the ordinary rebase-recovery
    /// session with the evidence as its guidance rather than parking outright: this path's
        ...
        ReviewContext context, RunAggregate run, StackAssessmentVerdict assessment, CancellationToken cancellationToken)
    {
        ...
        if (assessment.Kind == StackAssessmentVerdictKind.Undecidable)
        {
            string undecidableGuidance =
        ...
        CancellationToken cancellationToken)
    {
            ...
            await worktrees.AcquireRepositoryLockAsync(context.Project.RepositoryPath, cancellationToken))
        {
            ...
            catch (Exception exception) when (exception is TimeoutException or ProcessOutputStuckException)
            {
                await RestoreRebaseWorktreeBestEffortAsync(git, worktreePath, preRebaseHead, cancellationToken);
                logger.LogWarning(
                    exception,
    ...
    /// <para>
    /// Bounded by <see cref="MaxRebaseRecoveryRounds"/> (task: a run rebases its branch onto the
    /// current base branch, independent pre-PR review, cycle 1, both lenses): a recovery session
    /// that ends without a parseable resolution — or an undeclared, optimistically-accepted one —
        ...
        string rebasedFromCommit, CancellationToken cancellationToken)
    {
        ...
        if (outcome == ReviewFixOutcome.Disputed)
        {
            ...
            Exception? failure = await RunPaths.AppendDisputePositionAsync(disputeFile, summary, "Dispute position", cancellationToken);
            if (failure is not null)
            {
                logger.LogWarning(failure, "Could not write the pre-final-pass rebase dispute position to {FilePath}", disputeFile);
            }

            // Owed regardless of what resolves the conflict next, but no longer appended in its
            // own transaction here (independent pre-PR review, cycle 1, adversarial lens): the
            // assessment this dispute now runs before parking can take minutes, and an
            // interruption in that window used to leave this event on the stream with the run
            // still AwaitingRebaseRecovery — the exact shape that makes a later adoption replay
            // this same completed session's result and append a second one. Built once here and
            // carried down to whichever event finally moves the phase off AwaitingRebaseRecovery
            // (a resolved dispute, a final park, or an already-spent-assessment park), appended
            // in that same transaction — the identical atomicity the non-disputed outcome below
            // already keeps between its own TokensRecorded and PreFinalPassRebaseRecoveryCompleted.
            TokensRecorded tokensRecorded = result.ToTokensRecorded(runId, DateTimeOffset.UtcNow, model);
            ...
            RebaseGateOutcome assessedOutcome = await AssessOrParkAsync(
                ...
                (assessed, ct) => ActOnRebaseRecoveryDisputeAssessmentAsync(context, disputeParkText, tokensRecorded, assessed, ct),
                cancellationToken,
                onAssessmentAlreadySpent: (freshRun, ct) => RecordRebaseRecoveryDisputedParkAsync(
                    context, BothDiagnosesParkReason(freshRun, disputeParkText), tokensRecorded, ct));
        ...
        if (recordAsResolved)
        {
            ...
            // resolved it.
            session.Events.Append(runId, new RunRebasedOntoBase(
                runId, rebasedFromCommit, observedOntoCommit, WasNoOp: false, RecoveredByAgentSession: true,
                $"Resolved by a narrow recovery session (rebase-onto-main skill), from {ShortSha(rebasedFromCommit)} to {ShortSha(observedOntoCommit)}.",
                now));
        }
    ...
    /// on this run recorded (<see cref="BothDiagnosesParkReason"/>). Still owes the disputing
    /// session's own <paramref name="tokensRecorded"/> — appended together with the park's own
    /// completion event, in the same transaction, so a daemon restart between the two never leaves
    /// one without the other (independent pre-PR review, cycle 1, adversarial lens).
    /// </summary>
    private async Task<RebaseGateOutcome> RecordRebaseRecoveryDisputedParkAsync(
        ReviewContext context, string parkText, TokensRecorded tokensRecorded, CancellationToken cancellationToken)
    {
        await ParkAsync(
            ...
            [
                tokensRecorded,
                new PreFinalPassRebaseRecoveryCompleted(context.RunId, ReviewFixOutcome.Disputed, DateTimeOffset.UtcNow),
            ],
            cancellationToken: cancellationToken);
        return RebaseGateOutcome.Stop;
    }
    ...
    /// session to re-litigate a conflict this run's one assessment already looked at. Also owes the
    ...
    /// other exit on this path keeps (see <see cref="RecordRebaseRecoveryResultAsync"/>'s own doc).
    /// </param>
    ...
    /// <summary>
    /// The park message when <see cref="MaxRebaseRecoveryRounds"/> is reached (task: a run rebases
    /// its branch onto the current base branch, independent pre-PR review, cycle 1, both lenses) —
    /// a platform-imposed bound, not a claim that any one recovery session itself declared the
    /// conflict undecidable (that narrower claim is <see cref="RebaseRecoveryDisputedParkReason"/>'s
    /// own park). Parks from whatever phase this attempt was dispatched from (ordinarily
    /// <see cref="ReviewPhase.Settling"/>, or <see cref="ReviewPhase.RebaseRecoveryNeeded"/> for a
    /// round following an earlier human-resolved dispute) rather than a dedicated one — the same
    /// way <see cref="FinalFullPassCapParkReason"/> parks a repeatedly-reawakened track generically
    /// rather than through a track-specific phase. A human resolving with
    ...
    private static string RebaseRecoveryCapParkReason(RunAggregate run) =>
        ...
        $"{run.RebaseRecoveryRounds} time(s) in a row without ever landing cleanly — worth a " +
        "human's look rather than another automatic round. Check the worktree yourself " +
        "(`git status`, `git log`), then resolve with h9k review resolve --needs-fixes " +
        "\"<guidance>\" to dispatch an ordinary fix session carrying your guidance into this " +
        "worktree, which can still finish the rebase by hand, or finish the rebase yourself " +
        "in the worktree and describe what you did.";

    private static string RebaseRecoveryArtifactName(Guid sessionId) => SessionRoleName.PreFinalPassRebase(Short(sessionId));

    /// <summary>
    /// Whether the Settling phase's own mandatory gate is repair-eligible on the current tip (task:
    /// a pre-final-pass rebase that applies cleanly but breaks the mandatory gate gets a repair lap
    /// inside the same run instead of failing it): the most recently recorded
    /// <see cref="RunRebasedOntoBase"/> was a real rebase — clean or through the recovery session,
    ...
    private async Task<bool> DispatchSettlingGateRepairSessionAsync(
        ReviewContext context, RunAggregate run, string gateOutput, string? humanGuidance, bool enforceCap,
        CancellationToken cancellationToken)
    {
        if (enforceCap && run.SettlingGateRepairRounds >= _options.MaxSettlingGateRepairRounds)
        {
            await ParkOnSettlingGateRepairCapAsync(context, run, gateOutput, cancellationToken);
            return false;
        }
        ...
        // Reachable with no prior worktree check when a human's own `h9k review resolve
        // --needs-fixes` redispatches straight into ReviewPhase.SettlingGateRepairNeeded, the same
        // gap DispatchRebaseRecoverySessionAsync's own identical guard documents.
        if (worktreePath.IsBlank() || !Directory.Exists(worktreePath))
        {
            await FailAsync(
                context.RunId, context.TaskId,
                "The Settling-gate repair session cannot dispatch: the run's retained worktree is missing.",
                cancellationToken);
            return false;
        }
        ...
        DateTimeOffset dispatchedAt = DateTimeOffset.UtcNow;
        await using IDocumentSession session = store.LightweightSession();
        session.Events.Append(context.RunId, new SettlingGateRepairDispatched(
            context.RunId, sessionId, agent.ProcessId, agent.StartedAt, dispatchedAt, model, gateOutput, sessionName));
        await session.SaveChangesAsync(cancellationToken);
        logger.LogInformation(
            "Run {RunId}: Settling phase's mandatory gate failed on a real rebase — narrow repair session dispatched (session {SessionId}, pid {ProcessId}, model {Model})",
            context.RunId, sessionId, agent.ProcessId, model.Value);
        return true;
    }

    private async Task<bool> AwaitSettlingGateRepairSessionAsync(
        ReviewContext context, RunAggregate run, CancellationToken cancellationToken)
    {
        if (run.ActiveSettlingGateRepairSessionId is not { } sessionId
            || run.ActiveSettlingGateRepairProcessId is not { } processId
            || run.ActiveSettlingGateRepairProcessStartedAt is not { } processStartedAt)
        {
            await FailAsync(context.RunId, context.TaskId,
                "Run stream records an in-flight Settling-gate repair session without its identity.",
                cancellationToken);
            return false;
        }

        string streamFile = RunPaths.SessionStreamFile(CurrentRunDirectory(run), SettlingGateRepairArtifactName(sessionId));
        AgentResult? result = await WaitForSessionResultAsync(
            context.RunId, streamFile, processId, processStartedAt,
        ...
        {
            await FailAsync(context.RunId, context.TaskId,
                "The Settling-gate repair session died without a result.", cancellationToken);
            return false;
        }

        await RecordSettlingGateRepairResultAsync(context.RunId, result, run.ActiveSettlingGateRepairModel, cancellationToken);
        return true;
    }

    private async Task RecordSettlingGateRepairResultAsync(
        Guid runId, AgentResult result, AgentModel model, CancellationToken cancellationToken)
    {
        DateTimeOffset now = DateTimeOffset.UtcNow;
        await using IDocumentSession session = store.LightweightSession();
        session.Events.Append(runId, result.ToTokensRecorded(runId, now, model));
        session.Events.Append(runId, new SettlingGateRepairCompleted(runId, now));
        await session.SaveChangesAsync(cancellationToken);
        logger.LogInformation(
            "Run {RunId}: Settling-gate repair session completed ({Input}in/{Output}out tokens) — the mandatory gate re-runs on the next Settling entry",
            runId, result.TotalInputTokens, result.OutputTokens);
    }

    /// <summary>
    /// Parks on a spent Settling-gate repair cap (task: a pre-final-pass rebase that applies
    /// cleanly but breaks the mandatory gate gets a repair lap inside the same run instead of
    /// failing it) — <see cref="SettlingGateRepairCapReached"/> lands immediately ahead of
    /// <see cref="ReviewParked"/>, in the one transaction <see cref="ParkAsync"/>'s own
    /// <paramref name="precedingEvents"/>-shaped call carries, so
    /// <see cref="RunAggregate.ParkedFromReviewPhase"/> reads that phase by the time the park is
    /// applied — what lets a human's resolve route through this feature's own rules rather than the
    /// rebase-recovery cap's inherited ones.
    /// </summary>
    private async Task ParkOnSettlingGateRepairCapAsync(
    ...
    /// <summary>
    /// The park message when <see cref="DaemonOptions.MaxSettlingGateRepairRounds"/> is reached
    /// (task: a pre-final-pass rebase that applies cleanly but breaks the mandatory gate gets a
    /// repair lap inside the same run instead of failing it) — names both commits the triggering
    /// rebase ran between and the gate's own output, and offers the same two exits
    /// <see cref="RebaseRecoveryCapParkReason"/> offers its own cap: a needs-fixes resolve
    /// (routed by this feature's own dedicated <see cref="RunAggregate.Apply(Events.ReviewParkResolved)"/>
    /// branch — one more bought round, the counter left untouched, not the rebase-recovery cap's
    /// inherited ordinary-fix-session behavior) or a merge-ready resolve once the human has fixed it
    /// by hand.
    /// </summary>
    private static string SettlingGateRepairCapParkReason(RunAggregate run, string gateOutput) =>
        ...
        $" The gate's own output: {gateOutput} Check the worktree yourself (`git status`, `git log`), " +
        "then resolve with h9k review resolve --needs-fixes \"<guidance>\" to dispatch one more repair " +
        "session carrying your guidance, or fix it yourself in the worktree and resolve with " +
        "--merge-ready.";

    private static string SettlingGateRepairArtifactName(Guid sessionId) =>
        SessionRoleName.SettlingGateRepair(Short(sessionId));
    ...
    private async Task TerminateInFlightSessionsAsync(Guid runId, CancellationToken cancellationToken)
    {
        try
        {
            ...
            // The pre-final-pass rebase-recovery session (task: a run rebases its branch onto the
            // current base branch) is a third leg this crash-cleanup owes exactly the same
            // guarantee as the review passes and the fix session above: a crash while it is in
            // flight must not leave it running unread in a worktree the run has just failed
            // (independent pre-PR review, cycle 1, both lenses).
            if (run.ActiveRebaseRecoveryProcessId is { } rebaseRecoveryProcessId
                && run.ActiveRebaseRecoveryProcessStartedAt is { } rebaseRecoveryProcessStartedAt)
            {
                processManager.Terminate(rebaseRecoveryProcessId, rebaseRecoveryProcessStartedAt);
                logger.LogWarning(
                    "Run {RunId}: terminated the in-flight pre-final-pass rebase-recovery session (pid {ProcessId}) after the loop crashed",
                    runId, rebaseRecoveryProcessId);
            }

            // The Settling-gate repair session (task: a pre-final-pass rebase that applies cleanly
            // but breaks the mandatory gate gets a repair lap inside the same run instead of
            // failing it) owes this crash-cleanup the identical guarantee as its sibling leg just
            // above — a fourth leg, not covered by any of the other three checks in this method.
            if (run.ActiveSettlingGateRepairProcessId is { } settlingGateRepairProcessId
                && run.ActiveSettlingGateRepairProcessStartedAt is { } settlingGateRepairProcessStartedAt)
            {
                processManager.Terminate(settlingGateRepairProcessId, settlingGateRepairProcessStartedAt);
                logger.LogWarning(
                    "Run {RunId}: terminated the in-flight Settling-gate repair session (pid {ProcessId}) after the loop crashed",
                    runId, settlingGateRepairProcessId);
            }
        }
        catch (Exception exception) when (exception is not OperationCanceledException)
        {
    ...
    internal async Task ParkAsync(
        ...
        bool isInteractiveGate = false, IReadOnlyList<object>? precedingEvents = null,
        CancellationToken cancellationToken = default)
    {
        ...
        //
        // precedingEvents lands ahead of ReviewParked, in the same transaction — the same
        // ordering ReviewDisagreementParked already uses ahead of its own ReviewParked — so a
        // fact this specific park needs RunAggregate.ParkedFromReviewPhase (or another field) to
        // read is already on the stream by the time the park event's own Apply captures it
        // (task: a pre-final-pass rebase that applies cleanly but breaks the mandatory gate gets a
        // repair lap inside the same run instead of failing it — SettlingGateRepairCapReached is
        // the first user of this parameter).
        if (precedingEvents is { Count: > 0 })
        {
            session.Events.Append(runId, [.. precedingEvents]);
        }
    ...
    // internal, not private: tests build a ReviewContext through here rather than by hand, the
    // same "no git integration tests" fake-git testability RebaseGateOutcome's own comment
    // explains, without duplicating this method's own project/task/run load.
    internal async Task<ReviewContext?> LoadContextAsync(Guid runId, Guid taskId, CancellationToken cancellationToken)
    {
        await using IQuerySession query = store.QuerySession();
    ...
    private static SettleReason? MaySettleReason(RunAggregate run) => true switch
    {
        ...
        // conflict with judgment since the last fresh-context read, so "nothing owed" is not true
    ...
    /// </param>
    // internal, not private: tests construct one directly to exercise the stack-assessment methods
    // below without needing the real-git-dependent machinery (StackedParentWatch, a plain `git
    // rebase`) that gates their own only production call sites — see RebaseGateOutcome's own
    // comment.
