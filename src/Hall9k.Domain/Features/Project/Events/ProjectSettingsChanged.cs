...
public sealed record ProjectSettingsChanged(
    ...
    /// the same <see cref="ClaimGate"/> idiom. Optional, and defaulted for the same reason every
    /// other setting here is (System.Text.Json binds by name, not by position), so every stream
    /// written before this feature existed still replays unchanged.
    /// </summary>
    ...
    /// <see cref="MaxParallelTasks"/> uses. Optional, and defaulted for the same reason every other
    /// setting here is (System.Text.Json binds by name, not by position), so every stream written
    /// before this feature existed still replays unchanged.
    /// </summary>
