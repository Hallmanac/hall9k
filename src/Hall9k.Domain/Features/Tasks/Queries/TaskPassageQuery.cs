...
public static class TaskPassageQuery
{
        ...
        DateTimeOffset now)
    {
        ...
        //
        // Resolved once here — rather than inside FoldUntilMerged, which used to re-derive it on
        // each of the two calls below — so it can also ride out on TaskPassage itself: a
        // period-scoped throughput rollup (h9k status/project show) needs the merge's own instant
        // to know which period it falls in, and re-deriving it a second time outside this fold
        // would be the exact "build it twice" the passage task's own boundary note warns against.
        DateTimeOffset? mergedAt = taskType == TaskType.PrReview
            ? null
            : folds.Select(fold => fold.PullRequestMergedAt).FirstOrDefault(value => value is not null);
        PassagePhase mergeWait = taskType == TaskType.PrReview
            ? PassagePhase.NotApplicable
            : FoldMergeWait(completedWithPr, mergedAt, taskConcluded, folds, now);
        PassagePhase claimToMerge = taskType == TaskType.PrReview
            ? PassagePhase.NotApplicable
            : FoldClaimToMerge(taskEvents, completedWithPr, mergedAt, taskConcluded, folds, now);
    ...
    private sealed class RunFold
    {
        ...
        /// <summary>
        /// True once any recorded <see cref="VerificationPassed"/>/<see cref="VerificationFailed"/>
        /// on this run carried a null <c>GateDurations</c> — that field's own doc is explicit this
        /// means unobserved, never a claimed zero, so <see cref="GatesSum"/> alone cannot tell a
        /// gate-free run apart from one whose gate durations this stream simply never recorded
        /// (Copilot review, PR #335).
        /// </summary>
        public bool GatesUnknown { get; set; }
        public bool VerificationObserved { get; set; }
        ...
        foreach (IEvent recorded in run.Events)
        {
            ...
            switch (data)
            {
                case VerificationPassed passed:
                    ...
                    if (passed.GateDurations is null)
                    {
                        fold.GatesUnknown = true;
                    }
                    else
                    {
                ...
                case VerificationFailed failed:
                    fold.VerificationObserved = true;
                    if (failed.GateDurations is null)
                    {
                        fold.GatesUnknown = true;
                    }
                    else
                ...
                case PrReviewConformanceDispatched conformanceDispatched:
                    ...
                    // conformance lens, and the ReviewParked wait it leads into, out of "build"
                    // (Copilot review, PR #335: previously only ReviewParked closed this, which
                    // classified the whole adversarial-plus-conformance window as building).
                    fold.BuildEnd ??= conformanceDispatched.DispatchedAt;
                    break;
                ...
                case ReviewParked parked:
                    ...
                    // PrReviewEngine entirely. PrReviewConformanceDispatched above already closes
                    // BuildEnd for a run that dispatched a conformance lens, so this ??= is a
                    // no-op there; it still does the closing for a bare mention follow-up, which
                    // dispatches no lens session at all and so never gives
                    // PrReviewConformanceDispatched a chance to run first (independent pre-PR
                    // review, cycle 3, both lenses; narrowed by Copilot review, PR #335). Closing
                    // BuildEnd here too (harmless no-op via ??= on an ordinary FullPipeline run,
                    // whose BuildEnd is already closed well before any review park) stops that
                    // window from reading as "build", which double-counted it against the
                    // ReviewPark human wait below and kept growing "so far" while the run sat
                    // parked (independent pre-PR review, cycle 3, both lenses).
    ...
    private static TimeSpan SumGateDurations(IReadOnlyList<GateDuration> gates) =>
        gates.Aggregate(TimeSpan.Zero, (total, gate) => total + gate.Duration);

    // --- Gates: every recorded GateDuration across every run, unknown if any pass never recorded them ---

    /// <summary>
    /// <see cref="VerificationPassed"/>/<see cref="VerificationFailed"/>'s own <c>GateDurations</c>
    /// doc is explicit that a null value means the duration was never observed — never a claimed
    /// zero, the same convention <c>RunListItem.GateDurations</c> and
    /// <c>TaskShowCommand.FormatGateDurations</c> already draw for the identical field. A task
    /// that mixes an old stream missing the field with a newer one that has it can therefore never
    /// honestly report an exact total, so one unobserved pass anywhere makes the whole figure
    /// <see cref="PassagePhase.Unknown"/> rather than silently undercounting it as the sum of
    /// whatever happened to be recorded (Copilot review, PR #335).
    /// </summary>
    private static PassagePhase FoldGates(IReadOnlyList<RunFold> folds)
    {
        if (!folds.Any(fold => fold.VerificationObserved))
        {
            return PassagePhase.NotApplicable;
        }

        if (folds.Any(fold => fold.GatesUnknown))
        {
            return PassagePhase.Unknown();
        }

        return PassagePhase.Closed(folds.Aggregate(TimeSpan.Zero, (total, fold) => total + fold.GatesSum));
    }
    ...
    private static PassagePhase FoldMergeWait(
        IReadOnlyList<DateTimeOffset> taskCompletedWithPr, DateTimeOffset? mergedAt, bool taskConcluded,
        IReadOnlyList<RunFold> folds, DateTimeOffset now)
    {
        ...
        return FoldUntilMerged(taskCompletedWithPr.Max(), mergedAt, everHadPullRequest: true, taskConcluded, folds, now);
    }

    // --- Claim to merge: the first TaskClaimed to PullRequestMerged ---

    private static PassagePhase FoldClaimToMerge(
        IReadOnlyList<IEvent> taskEvents, IReadOnlyList<DateTimeOffset> completedWithPr, DateTimeOffset? mergedAt,
        bool taskConcluded, IReadOnlyList<RunFold> folds, DateTimeOffset now)
    {
        List<DateTimeOffset> claims = [.. taskEvents.Select(recorded => recorded.Data).OfType<TaskClaimed>().Select(claimed => claimed.ClaimedAt)];
        if (claims.Count == 0)
        {
        ...
        return FoldUntilMerged(claims.Min(), mergedAt, completedWithPr.Count > 0, taskConcluded, folds, now);
    }

    /// <summary>
    /// The shared close for both <see cref="FoldMergeWait"/> and <see cref="FoldClaimToMerge"/>,
    /// anchored on <paramref name="mergedAt"/> — <see cref="Compute"/>'s own single resolution of
    /// whichever run's stream recorded <see cref="PullRequestMerged"/>, since there is at most one
    /// across a task's whole run history (the branch and its one pull request persist across every
    /// follow-up lap, <c>TaskReopened</c>'s own doc). Unknown rather than open once the task has
    /// concluded with no merge ever recorded: nothing further will ever close this interval, so
    /// leaving it "open" would grow forever on a task that is done.
    ...
    private static PassagePhase FoldUntilMerged(
        DateTimeOffset anchor, DateTimeOffset? mergedAt, bool everHadPullRequest, bool taskConcluded,
        IReadOnlyList<RunFold> folds, DateTimeOffset now)
    {
        if (mergedAt is { } merged)
        {
            ...
            // cycle 1, both lenses).
            return PassagePhase.Closed(Clamp(merged - anchor));
        }
