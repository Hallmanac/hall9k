...
public static class ProjectHomePaths
{
    ...
    /// <summary>
    /// This project's orchestrator recipes (task: an operator starts a lean node or project
    /// orchestrator window). <see cref="RecipeLibraryPaths"/> is the node's own copy of this same
    /// shape at <c>~/.hall9k/recipes</c>; the two are siblings, not one shared directory, because a
    /// project window and the node window are different orchestrator kinds with different contracts.
    /// </summary>
    public static string RecipesDirectory(string home) => Path.Combine(home, "recipes");

    /// <summary>
    /// The platform-owned anchor every launch line appends as this window's system prompt
    /// (<see cref="RecipeLibraryPaths.LaunchAnchorFileName"/>): overwritten on every render, never
    /// hand-edited, never generated by the orchestrator-recipe-generator skill.
    /// </summary>
    public static string LaunchAnchorFile(string home) => Path.Combine(RecipesDirectory(home), RecipeLibraryPaths.LaunchAnchorFileName);

    /// <summary>
    /// The settings file the launch line rides on with <c>--settings</c> (command-line scope,
    /// never <c>.claude/settings.json</c> — see <see cref="RecipeLibraryPaths"/>'s own doc for why).
    /// Platform-owned like the anchor.
    /// </summary>
    public static string RecipeSettingsFile(string home) => Path.Combine(RecipesDirectory(home), RecipeLibraryPaths.SettingsFileName);
    ...
    /// beside the anchor rather than under the ordinary <see cref="SkillsDirectory"/> — the one
    /// skill this platform ships into <c>recipes/</c> instead of <c>skills/</c>, so opening the
    /// recipes folder shows the anchor, the generator, and (once it has run) the recipes it wrote.
    /// </summary>
