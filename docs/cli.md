...
triage block, putting a contradiction in the run log. `h9k pr reply-guard` is not a command you
type: it is the PreToolUse hook a follow-up session launches with, refusing the `gh` routes into a
review thread so the sanctioned one is the only one left. `gh pr comment` is deliberately not
...
└── notes/      seeded alongside it: prototype-feedback.md holds dated recipe feedback; waiters/
              holds the background waiter script(s) this window arms at start-up, plus their
              offset and gh-streak state
```
