...
| `h9k config set --effort <low\|medium\|high\|xhigh>` | Sets the reasoning effort level every headless agent session runs at (the daemon's dispatches, `h9k task start` and `h9k task delegate`), one node-wide level, with `default` clearing it. It exists because a headless session ignores the owner's user-level `effortLevel` and honors the `--settings` file the daemon hands it, and Claude Opus 5.5 defaults to medium where earlier Opus models defaulted to high. |
