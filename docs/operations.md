...
| "`owner/repo#N`: `<login>` tagged you — …" | A **pr-review** task whose pull request GitHub reports a fresh comment mentioning this install's own login on — auto-pr-review's second trigger (Decisions Log PLACEHOLDER-ed6044a5). Whether that mention minted this task or attached to a live one, the daemon dispatches a bounded follow-up lap that reads the tagged comment against the review already done and parks an addendum beside the report; the line names the pull request, who tagged you, and the first line of their comment | `h9k task show <id>` for the addendum path, then walk it with the `walk-pr-review-findings` skill and resolve with `h9k review resolve <id> --merge-ready` once the drafted reply is directed |
