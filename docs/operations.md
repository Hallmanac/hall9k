...
asked of you while the ball is in the author's court, and the moment it comes back the row moves to
needs-you with the line above. It is counted under **Delivered** in the rollups — at that
granularity both mean "an open pull request this install is watching" — and reached by
`--state waiting` for the column, `--state attention-waiting` for the group.
...
| `h9k pr approve <task> --note "…"` / `h9k pr request-changes <task> --note "…" [--finding "<path:line: text>"]…` | A **pr-review** task whose review lap you are in (`h9k pr review`), and you have reached your verdict. Not a recovery lever — the ordinary end of a lap — listed here because it replaces `review resolve --merge-ready` for somebody who actually reviewed rather than merely walked the report. | Posts the GitHub review on the pull request's **current head** under your own login, the changes-requested one with every `--finding` as a line comment, then records the verdict, releases the worktree and parks the task on the pull request as **AwaitingAuthor** to wait for its author, exactly as `review resolve --merge-ready` does (Decisions Log #160): the closeout watcher polls it, flags it needs-you the moment the author replies, pushes, or re-requests your review — `h9k pr review <pull-request> --since-my-review` reads only what changed — and it reaches Done when every thread you opened is resolved or the pull request merges or closes. A pull request carrying more review threads than GitHub's own 100-thread page returns never reaches Done that first way — "every thread" would be a claim about the first hundred — so it stays waiting until it merges or closes, or you stop watching with `h9k task abandon <id>`, which records the walk-away rather than Done. `h9k task resolve` is **not** a lever here: as the row above says, it is the attestation exit from a Failed task alone and is refused on a waiting review. The post comes first, so a post that fails records nothing and you re-run. GitHub rejects the whole review when a finding names a line its diff does not contain — nothing is posted until every line is one it accepts. `review resolve --merge-ready` still closes a pr-review task out when the report was walked and nothing needs posting. |
