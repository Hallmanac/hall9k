...
| "`owner/repo#N`: `<login>` tagged you — …" | A **pr-review** task whose pull request GitHub reports a fresh comment mentioning this install's own login on — auto-pr-review's second trigger (Decisions Log PLACEHOLDER-ed6044a5). A mention that *minted* this task has its answer folded straight into the ordinary findings report, under its own "You were asked" section — there is no separate lap, since the mint's own review session was asked to answer it directly. A mention that *attached* to an already-live task instead gets a bounded follow-up lap that reads the tagged comment against the review already done and parks an addendum beside it. Either way the line names the pull request, who tagged you, and the first line of their comment | `h9k task show <id>` for the report or addendum path, then walk it with the `walk-pr-review-findings` skill and resolve with `h9k review resolve <id> --merge-ready` once the drafted reply is directed |
...
PLACEHOLDER-ed6044a5), because a task that closed out just because it had nothing left to watch
...
| **release** | `h9k task release <id> [--keep-interactive]` | The machine, same as `handback`, unless `--keep-interactive` is given — the one way to hand a task back headless while it still parks at every boundary. |
