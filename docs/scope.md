...
session dispatched inside the same run (never a task reopen); only a conflict that session cannot
honestly resolve, or the exhaustion of its own round cap, parks for a human (PLAN.md Decisions Log
#138). A rebase that applies cleanly but
...
own node file, but it also has to be currently allowed by the project's own ledger chain, bound to
...
takes it is decided by rank then oldest assignment (Decisions Log #188), no
...
#188): a follow-up lap on a task already past its first pull request outranks a
...
a session's own skill list (PLAN.md §16 #175). `--from-release` refuses a payload
...
confirmation names the scope in numbers (tasks, runs, ideas, and epics) and the deadline, and refuses under
...
file beside it instead). Launch text (the exact command an operator pastes to start a
