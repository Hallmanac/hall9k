...
h9k daemon autostart enable|disable   # start-at-login, strictly opt-in and never implied by install or start: a launchd LaunchAgent on macOS, a Task Scheduler logon task on Windows, never a service (Decisions Log #3). enable also captures this shell's PATH and HALL9K_* overrides into the registration, since neither service manager gives the daemon an environment of its own, and it is the only thing that rewrites a Windows registration's launch script — re-run it once on a machine registered before the launcher opened the daemon's log for it (§16 PLACEHOLDER-d4e64dfa)
h9k daemon autostart launch --binary <h9kd> --log <h9kd.log>   # Windows only, and not a command to type: the vehicle the registered logon task runs through wscript.exe and cmd.exe. It opens the log with FILE_APPEND_DATA and a share mode that admits readers, other writers and a delete, hands that handle to h9kd as its stdout and stderr, closes its own copy, then waits for the daemon and exits with its exit code so restart-on-failure still sees a crash. It exists because a handle cannot be passed through the VBScript command line the registration composes (§16 PLACEHOLDER-d4e64dfa)
h9k config show|set          # the daemon's durable operating settings: node ceiling (--max-concurrent-task-runs), the per-run session cap default (--session-cap-per-run), model-by-role, interactive-claim-stale-after-days, review-cycle caps, the review stage composition (--review-stage-composition, --accept-reduced-review to degrade it), a periodic token-spend budget (--spend-budget, --spend-period; backlog 59, Decisions Log #103, #111, #112, #120, #129), the message sweep's active/idle poll ranges (--message-poll-active-min/-max, --message-poll-idle-min/-max, seconds; idea 202383dc M1b), the seconds a non-leader fleet node holds a review request before minting for it itself (--auto-pr-review-mint-hold, 300 by default, 0 never defers), the invite expiry window (--invite-expiry-hours; idea 202383dc, T2)
h9k doctor [--yes] [--no-configure]  # diagnose the database situation and what to do about it; --yes remediates non-interactively, for scripts and dispatched agents; --no-configure withholds the one remediation that guesses, recording no connection string where none resolves (what h9k update --restart passes) (Decisions Log #73, #74, #118)
...
h9k project join <name> [--owner <fingerprint>] [--invite <secret>]   # establish or confirm this node's identity in a project's ledger: generates this node's ed25519 key once under ~/.hall9k/keys/<node-id>, then writes its node file signed with it; the first join naming no --owner establishes this owner's root and that fingerprint becomes the owner id everywhere in Hall9k (h9k owner show <fingerprint>, h9k task assign --owner <fingerprint>), recorded on the Owner stream, and a later plain join keeps whatever root is already claimed; that same genesis commit also mints this project's own key (idea 202383dc, M2, Brian's ruling 2026-09-17: a 26-character ULID, Cysharp's Ulid, never the genesis owner's own root fingerprint), which this install's own local Project stream records too (h9k project show prints it) and which every envelope this project ever sends now carries; --owner <fingerprint> claims an existing root instead, unverified until an already-enrolled node of that owner confirms it (h9k node vouch, or a matched h9k node invite); --invite <secret> proves possession of a single-use secret from h9k node invite/h9k project invite instead, writing a proof the minting node's own daemon sweep matches and vouches without a further prompt (refused together with --owner; idea 202383dc, T2); a join that establishes no root of its own (an --owner claim, an --invite join, or a later plain join into an already-established project) still reads this project's own key back from the live ledger and records it locally when the ledger already has one; re-runnable to change the claim, retiring a self-created root (required in the project being joined, best-effort in every other reachable, non-archived project that holds it) when this node turns out to belong to another one (idea 202383dc, A2a, §16 #190); refused before any key is generated or any ledger byte is written when the project's own GitHub account has no push on the repository, naming the repository and the rule (idea 202383dc, A2b, §16 #197); adopting a project whose ledger predates this chain onto a team is safe in any join order but one: every install updates first, then the existing owner runs a plain h9k project join <name> (no --owner, no --invite) once to become genesis (and, on a ledger whose own genesis predates the project key, runs h9k project assign-key <name> once to backfill it); only after that does h9k project invite mint anything (refused, naming this same command, while no membership is recorded yet), and an invited teammate runs h9k project join <name> --invite <secret> against the project they already registered, keeping their own registration, home, and history untouched; --invite never self-writes genesis, whatever the members folder's own state, so the one order that is never safe is a plain join with no --invite landing on the new build before the owner's own first join has (idea 202383dc, T2 criterion 3, §16 #209)
h9k project assign-key <name>   # one-time backfill of a project's own key (idea 202383dc, M2; Brian's ruling 2026-09-17) for a ledger whose genesis predates it: mints a fresh 26-character ULID and writes it, signed, onto the genesis fingerprint's own members/<fingerprint>.yaml; refused for anyone but the genesis owner and refused again once a key already exists; run it once, before the first h9k project invite on an adopted project, and every other install picks the key up the next time it runs h9k project join there
...
h9k project show <name>      # one project: home, registration, settings, rollup, newest tasks, and its own generated project key (idea 202383dc, M2) once a join has read one back from the ledger
h9k project remove <name> [--reason <TEXT>] [--yes]   # archive a project: hides it from h9k project list, stops the dispatcher and every daemon sweep acting on it — reversible, nothing deleted, home directory untouched (Decisions Log #182)
h9k project remove <name> --purge [--reason <TEXT>] [--yes]   # archives (if not already) and schedules a permanent hard delete of the project's database footprint 24 hours out — the one exception to the platform's nothing-is-deleted doctrine. Refuses under the same task-state predicate a plain archive does; the confirmation names the scope in numbers (tasks, runs, ideas, epics) and the deadline before anything is scheduled. Visible while pending in h9k project list --include-archived and h9k project show; reactivating is refused until the purge is cancelled. A daemon sweep carries it out, on the next start if the deadline passed while the daemon was down (Decisions Log #185)
...
h9k project set <name> --design-review-drive on|off   # whether the DESIGNER persona's review may stand this project's product up and drive it through browser automation, or reads the diff and the design files alone (idea b9b09779 piece 3, Decisions Log #249). Default ON, and h9k project show prints it with its origin (explicit or default) exactly as it does auto-pr-review, for the same Decisions Log #161 reason: with a default of on, a stored 'off' a reader could take for the platform's own initial state is how a setting goes unnoticed. Driving ALSO needs a run skill on this project's ledger — with none there is nothing that says how to stand the product up, so the review is static whatever this setting says, and the report names which of the two reasons applied. The QA persona's own --qa-review-drive below is the same setting with the opposite default, OFF
h9k project set <name> --qa-review-drive on|off   # whether a QA-persona review of a pull request here may LAUNCH the running product and drive it through browser automation. Default OFF: the QA review reads the diff, builds its blast-radius map, and runs the project's end-to-end tests on the review worktree, and any verdict that needs the running product becomes a human walk-through it writes out step by step. 'on' lets that session start the product from the run skill on the project's ledger, on an ephemeral port it reports and tears down, drive the changed flows, and put each screenshot beside the finding it supports; the report then carries a Driven section naming the flows walked. A project with NO run skill drives nothing whatever this says, because there is nothing to launch it with. h9k project show prints the value and whether it was chosen or is the default. The designer's --design-review-drive above is the same setting with the opposite default, ON (idea b9b09779 piece 2, Decisions Log #253)
...
h9k project set <name> --orchestrator-feed actionable|transitions|everything   # how much of this project's own history h9k orchestrator feed hands an orchestrator window. Three nested bands: actionable is only what somebody is owed (parks and disputes, gate and run failures, a merge that stays failed, daemon trouble, and any message from a person or another node's window), transitions (the default) adds task state changes, ideas logged or updated, and claims or takeovers involving another node, and everything adds a run's own phase changes. A message from a person is admitted at every band. The band decides what the feed shows, never what the platform records (#241)
h9k orchestrator feed --project <name> [--drain] [--since <TIME>]   # what happened while no window was live: everything past this project's own feed cursor that its band admits, oldest first, grouped by task, each as one plain line the feed's own table of event type to wording composes for it. Not a second store — a cursor over this node's own event log plus a filter, so nothing is buffered and nothing is lost by not reading it. A plain read is repeatable; --drain is the explicit "I have read these" that moves the cursor; --since (45m, 6h, 3d, 2w, or an instant) reads history and never moves it, and the two flags are refused together (#241)
...
h9k project set <name> --discover-run-skill   # ask the daemon to (re)discover how this project is stood up locally; h9k project add asks once at registration, this is how you ask again after the repository's launch story changed or an earlier discovery failed. Records only the ask; the daemon surveys the repository and dispatches a read-only session on its next run-skill sweep (#246)
h9k project run-skill show <name>   # print this project's run skill as this node's stream last recorded it, with who composed it, when, and against which commit; a project with none says what it is waiting on (#246)
h9k project run-skill set <name> --file <path> --shape pointer|full-text|none-discoverable [--against-commit <SHA>]   # replace the run skill by hand, through the same event a discovery session's answer comes back through; the file must carry all six shared sections and the shape line is written for you (#246)
...
h9k owner show [<owner>]     # this owner's own record, including its linked accounts as its GitHub connections (login and GitHub's own numeric id, or 'unconfirmed' when gh has never answered for it) — an identity holds a list of accounts, so more than one shows if more than one is registered (idea 202383dc, A2b, §16 #197)
```
...
envelope itself carries the project's own generated key (idea 202383dc, M2; Brian's ruling
2026-09-17): a 26-character ULID `h9k project join` mints once, writing this project's genesis
members commit, and never the genesis owner's own root fingerprint (two projects can share one
genesis owner, since `TrustChain.GenesisRootFingerprint` is an owner fact alone, and must not share
a wire key). Identical on every node that shares the repository and never any one install's own
locally-minted project id, since two nodes registering the same shared repository mint two
different, uncorrelated ids for what is really one project. A receiver compares the envelope's own
key directly against the local project whose ledger the read is scoped to (`TrustChain.ProjectKey`,
or this install's own recorded key when no live trust chain was computed this tick), refusing and
naming it in `h9k status` when the two disagree; when this install does not yet know its own key it
falls back to a plain store lookup, refusing a hit that names a different local project instead. A
null key, or one not shaped like a 26-character ULID (a legacy sender), is read normally, never
refused as malformed. A ledger whose genesis predates this piece has no key until its owner runs
`h9k project assign-key` once.
...
this install's own project for an unrecoverable hard delete with no decider in the way). The
events envelope carries the project's own generated key exactly as an ordinary message does
(above), and each replicated event's own origin headers carry it too, beside owner root, node,
event id, and origin sequence; `EventReplicationInbox` resolves the local project by that key the
identical way, refusing and naming a mismatch in `h9k status`. Which events travel
...
#236). Only two of the three node states catch up by themselves: a brand-new node
...
that. `h9k task pull <task-id> [--project <PROJECT>] [--again]` asks every member for one stream by
id, with no tracker item involved (the full id, or the short id off a board row, resolved against
this project's ledger records, which also say which project to ask); `h9k project pull <project>
--since <global-sequence|all>` asks for a whole project's
history from a bound read on the *answering* node, and `--since` is required because the two are
very different asks. Both are broadcasts, both queue one envelope and return (a task pull touches
git only when it has to read the ledger to resolve a short id or pick a project), both report the
same outcomes the adoption refusal does, and `h9k status` shows the request while it stands. A
request a peer declined or answered is closed rather than in flight, so a re-run asks again and
...
**One owner's own fleet reconciles itself** (task 252bc5cf, Decisions Log #262).
...
agent and written by the daemon** (idea b9b09779, piece 4; #246). `h9k project
...
(idea b9b09779 piece 1, Decisions Log #239) — see `h9k owner set --persona` above
...
**The QA persona's own review** (idea b9b09779 piece 2, Decisions Log #253) reads
the pull request through blast radius rather than through correctness, which the engineer's review
is already doing on the same diff. Its first job is a map, in plain language, as the report's first
section: the behaviours the diff changes, the adjacent behaviours sharing a code path or data with
them, and the user-facing flows crossing either. Every entry on the map carries exactly one of
three verdicts — covered by an existing automated test it names, needs a new automated end-to-end
test it specifies in a sentence, or needs a human walk-through it writes out step by step — and
every later finding cites the map entry it came from. The session runs the project's end-to-end
tests on the review worktree, scoped to the blast radius where the test layout allows and in full
otherwise, and reports pass, fail or absent with its evidence; a failure is quoted rather than
summarized. It also checks the change against the project's stated conventions, the writing
conventions, the decision log and the acceptance criteria on whatever the pull request is linked
to, naming each met, not met, or not verifiable from the code alone. Whether it may launch and
drive the running product is `--qa-review-drive` above, off by default. The report ends with an
offer to run the branch locally so the reviewer can walk it, present only when the project has a
run skill and phrased with the walk-throughs as the suggested route — a question in the report,
never an action.

**The designer's own review** (idea b9b09779 piece 3, Decisions Log #249) is one
session answering seven lenses in a fixed order: user experience, conformance to the proposed
design, motion (transitions and animations), CSS practice, accessibility, look and feel, design
system. The platform composes that persona's report section rather than carrying the session's
file in verbatim, because three things in it are the platform's to say: the fixed lens order, with
a one-line stand-in for every lens the session left alone so a silent section is never read as a
clean one, naming the usual reason a lens goes unanswered rather than asserting it of this
change; whether the review drove the product, from what the run recorded at
dispatch; and the closing offer to run the branch locally for the reviewer, present exactly when
the project has a run skill, a question the reviewer answers and never an action the session then
takes. The proposed design is a Figma link, an image set, or a prototype named on the linked task
or issue or in the pull request body — with none named the conformance lens reports `no reference
supplied` and judges nothing, while every other lens still applies in full. The session looks for
a design system in the repository (tokens, a component library, a documented system) and names
what it found or that there is none; each design-system finding cites the token or component it
concerns. With `--design-review-drive on` (the default) AND a run skill on the ledger, the session
launches the product on the review worktree on an ephemeral port it reports, walks the changed
user-facing flows, runs an automated accessibility audit on each walked screen, tears the app
down, and the report carries a `Driven` section citing each screenshot beside the finding it
supports; with the setting off or no run skill the review is code-and-design-file only, the
accessibility lens's findings are marked static, and the report says which of the two applied.
