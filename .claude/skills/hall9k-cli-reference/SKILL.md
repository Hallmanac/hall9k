...
h9k project add --name <n> --repo-url <url>   # register a project and create its home directory; offers to reactivate or rename an archived project whose name it collides with (--reactivate-archived, --rename-archived-to <NAME>) (Decisions Log #182); also runs h9k project join once the repository is reachable on disk, reporting plainly if it could not (§16 PLACEHOLDER-126107bc)
h9k project join [<name>] [--owner <fingerprint>]   # establish or confirm this node's identity in a project's ledger: generates this node's ed25519 key once under ~/.hall9k/keys/<node-id>, then writes its node file signed with it; a join naming no --owner establishes this owner's root and that fingerprint becomes the owner id everywhere in Hall9k, recorded on the Owner stream; --owner <fingerprint> claims an existing root instead, unverified until the team half's vouch (not yet built); re-runnable to change the claim, retiring a self-created root when this node turns out to belong to another one (idea 202383dc, A2a, §16 PLACEHOLDER-126107bc)
