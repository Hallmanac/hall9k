...
`in_reply_to` / `commit_id` / `event=` / `body=`, every `addPullRequestReview…` mutation
...
this repository, and matching the route alone refused a `git grep` for what it never sent. The hook
fails **open** everywhere (missing binary, unparseable payload, a tool that is not Bash): blocking
