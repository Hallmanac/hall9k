...
h9k project set <name> --design-review-drive on|off   # whether the DESIGNER persona's review may stand this project's product up and drive it through browser automation, or reads the diff and the design files alone (idea b9b09779 piece 3, Decisions Log #PLACEHOLDER-cd722750). Default ON, and h9k project show prints it with its origin (explicit or default) exactly as it does auto-pr-review, for the same Decisions Log #161 reason: with a default of on, a stored 'off' a reader could take for the platform's own initial state is how a setting goes unnoticed. Driving ALSO needs a run skill on this project's ledger — with none there is nothing that says how to stand the product up, so the review is static whatever this setting says, and the report names which of the two reasons applied. The QA persona's own drive setting lands with piece 2 and defaults OFF
...
**The designer's own review** (idea b9b09779 piece 3, Decisions Log #PLACEHOLDER-cd722750) is one
