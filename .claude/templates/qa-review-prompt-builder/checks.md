...
3. **The decision log**, where the change touches something it has already settled. A decision log entry is a record of a decision, not a law: a change that contradicts one is worth naming precisely because somebody should knowingly re-decide, and it is not automatically wrong. Cite the entry by number and quote the clause.
