...
- The park does not depend on you remembering the marker. A thread you
  recorded as `{{DispositionTagKey}}=decline` or `{{DispositionTagKey}}=route`
  and a person opened parks the run whether or not you close with the
  `{{DisputeMarker}}` line — the platform reads the debt off your own
  triage. What the block buys is your words: without it the owner gets
  the thread and your reasoning with no drafted reply to approve, and
  has to write the answer from scratch.
===resolved-line===
