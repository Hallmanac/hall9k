...
18. **Slice-1 gaps adopted.** Minimal **Project entity** + `h9k project add` (repo path, base branch, verify commands — the daemon must learn these from somewhere; the v0 CLI sketch lacked it). `h9k task add --file task.md` (frontmatter + body) + stdin — the readiness contract doesn't fit CLI flags. Per-node **concurrency cap**, default 3 (§6.4 guidance). Daemon **retry/backoff on Postgres connect** (launchd starts `h9kd` before Docker Desktop at boot).
