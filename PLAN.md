...
25. **Failed tasks get an explicit human-driven retry** (2026-08-17): `h9k task retry <id> --reason` appends `TaskRetried` (Failed to Queued), amending #20's "Failed stays a dead end" guardrail; Abandoned remains one. Origin incident: the first two automatic follow-up runs completed their work and passed the gates, then failed at the daemon's push step (plain push against their rebased branches; backlog 08), and the completed, gated work sat stranded in the worktrees because nothing left Failed. Failure of the machinery around the work must not permanently condemn the work. Shape: retry is a new event, never a rewrite; the stream reads added, failed, retried, claimed, and `h9k task show` keeps the failure reason alongside the mandatory retry reason (the human states why another attempt should go differently). Retry is **human-only**: no monitor path appends it, per the #11 never-loop-on-judgment rule; being human-initiated it resets the closeout budget like a manual reopen (#22) rather than spending it. Distinct from `TaskReopened` (Done-only, PR follow-ups): the decider guards differ and the launcher prompts differ. The retried task re-enters the unchanged sweep/claim/launch pipeline, so the next claim increments the lease generation as usual (#7). The event carries the failed run's branch as observed at dispatch (`FollowUpKind.Retry` marks the mode); the launcher resumes a surviving branch/worktree through the #20 `CheckoutExistingAsync` path but hands the agent the **standard** task prompt (the agent picks up whatever committed work survives), and when the artifacts are gone it starts clean from the base branch instead of failing the launch (a reopened task gets no such fallback: its branch IS the pull request). A clean start dispatches as a non-follow-up, so its completion opens a fresh pull request instead of adopting a stale PR URL still on the task; recording the old PR as the fresh branch's own would be a guessed audit fact, and it would leave the retried work monitored by nothing.
