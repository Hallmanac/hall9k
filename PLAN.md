...
The two connectors are deliberately not symmetrical. Jira reads through `IWorkItemProvider` like GitHub does, but writing it is not configuration-agnostic the way GitHub's is, because issue types, required fields and routing rules are one organisation's configuration (Decisions Log #102): `h9k task push-to-jira` dispatches an agent session that reads those rules from the project's own repo skills and composes a payload (create, update, or comment) rather than calling Jira itself, and `h9k task write-jira` is the sole executor of every Jira write, validating the payload, executing it against the Jira Cloud v2 REST API with the same registered connection the read side uses (Decisions Log #114), and recording the outcome only after reading the created or updated item back. `h9k task link-jira` remains the way to record a pre-existing card the platform did not create.
...
The log this section carried is platform data now (idea d805fd8b). Every decision is an event in
the store, and the file to read is `decisions.md`, which the daemon renders into this project's
home and into every worktree at dispatch. Record one with `h9k decide "<one claim>"`, read them
back with `h9k decide list` and `h9k decide show <id>`, and cite one by the id in its heading
rather than by a sequential number somebody has to assign at merge time.

Every entry that stood here was imported keeping the citation it already had (`h9k decide
import`), so a reference written anywhere in this repository as **Decisions Log #62** still
...
The heading keeps its number so that §17 stays §17 and every cross-reference to a later section,
here and in `docs/`, keeps pointing where it always did.
