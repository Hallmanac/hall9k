...
- The runs table shows each run AS IT CURRENTLY SITS - every cell answers now, and history lives in the drill-down (the table philosophy ruled 2026-08-24): a Phase column joins the table showing each live run's current phase in the phase line's own vocabulary (building, gates, review cycle 2), because by the time a reader reaches the table the task-level phase line has scrolled out of view; terminal runs show their outcome phase
- The table's existing Model column shows the model of the run's CURRENT session - the one State and Phase are describing - so an UnderReview row names the reviewing model, not the build model it shows today; a terminal run shows its last session's model by the same rule, and a compact marker appears whenever the run's sessions spanned more than one model (origin incident 2026-08-24: the row read State UnderReview beside Model claude-sonnet-5 while both review lenses ran claude-opus-5[1m], and Brian reasonably read the role split as violated)
- A tokens column shows each run's running total across ALL its sessions so far (build, reviews, fixes - the honest cost of the attempt, growing live), plus a task total across runs, reading from the TokensRecorded events / RunDetails projection that already exist - display composition, no new capture
