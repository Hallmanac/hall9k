...
- The table's existing Model column (it already shows the dispatch-recorded build-session model) gains a compact marker when any of the run's other sessions (review lenses, fix runs) ran a different model - the row stays glanceable, the marker says there is more story, and the --run drill-down is where the per-session breakdown lives (ruled at the 2026-08-24 walk: a run is not one model, so the cell names the primary and never pretends otherwise)
