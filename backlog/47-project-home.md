...
- h9k project add (and an init/adopt path for a registered project without a home) creates the shape with platform code end to end, no agent: AGENTS.md, repo/, ideas/, tasks/, runs/, skills/, and a .claude/ adapter directory - the recipe ruling from the installation sitting applies ("this is a recipe, not an agent"). Init/adopt ALWAYS materialises fresh from the recorded remote: an existing clone elsewhere on the machine is inconsequential (git is distributed, including across one disk), and moving work out of an old location is the cutover chore's agent-assisted job, never this recipe's
...
- The install owns the canonical skill set at ~/.hall9k/skills, published by h9k install (sourced from this repo's .claude/skills today, from release artefacts once backlog 42 lands) - the IDEA-skill-layer Tension 8 ruling that machinery-required skills ship with the install, as self-contained as the binaries
- skills/ is seeded from that canonical set with .claude/skills/ generated as symlinks into it, so h9k install updating the platform updates every project's platform skills; project-specific skills sit beside the seeded ones and survive re-seeding
- The agent briefing composes the project home into the dispatch prompt: when the task's project has a home, the prompt names the home's skills/ and the generated AGENTS.md alongside the repo skills DiscoverRepoSkills already hands over - a dispatched agent is told where everything lives and never hunts
