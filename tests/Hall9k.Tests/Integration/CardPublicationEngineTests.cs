...
public sealed class CardPublicationEngineTests(PostgresFixture postgres) : IClassFixture<PostgresFixture>, IDisposable
{
    ...
    /// <para>
    /// That work runs detached and only after the dispatch has been recorded, because that is the
    /// real order and the engine depends on it: the dispatch append is fenced on the task's
    /// stream version, so a session that wrote to the task before the daemon finished recording it
    /// would be racing its own launch. Waiting on the recorded dispatch rather than on a delay
    /// keeps the test watching a condition instead of guessing at a duration.
    /// </para>
    /// </summary>
        ...
        Func<Task>? beforeFinishing = null) : IExecutor
    {
        ...
        public Task<SpawnedAgent> SpawnAsync(AgentSpawnRequest request, CancellationToken cancellationToken)
        {
            ...
            _ = Task.Run(async () =>
            {
                ...
                {
                    await WaitForRecordedDispatchAsync(cancellationToken);
                    await work();
                }
        ...
        private async Task WaitForRecordedDispatchAsync(CancellationToken cancellationToken)
        {
            for (int attempt = 0; attempt < 100 && store is not null; attempt++)
            {
                ...
                if ((await query.LoadAsync<TaskDetails>(taskId, cancellationToken))?
                    .PublicationSessionDispatched is true)
                {
                    return;
                }
