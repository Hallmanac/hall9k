...
public sealed class ConnectionRegistrationTests
{
    ...
    [Fact]
    public void A_jira_connection_cannot_borrow_the_gh_cli_login_because_it_holds_no_jira_token()
    {
        // gh-cli names something real, so the "names nothing" rule above lets it through: the
        // thing it names is a GitHub session, which can never present a Jira token. Registering
        // it would put a connection on the stream that fails at the first card read. Origin: the
        // pull-request review of the Jira branch (2026-08-22).
        Action register = () => RegisterJira(CredentialReference.GhCli);

        register.Should().Throw<DomainValidationException>().WithMessage("*gh CLI's login*");
    }
