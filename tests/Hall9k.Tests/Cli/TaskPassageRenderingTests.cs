...
public sealed class TaskPassageRenderingTests
{
    ...
    [Fact]
    public void Gates_unknown_renders_the_word_unknown_rather_than_omitting_the_row()
    {
        TaskPassage passage = Empty with { Gates = PassagePhase.Unknown() };

        TaskShowCommand.ComposePassageLines(passage)[0].Should().Contain("gates unknown");
    }
