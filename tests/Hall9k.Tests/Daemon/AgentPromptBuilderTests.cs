...
public sealed class AgentPromptBuilderTests : IDisposable
{
    ...
    public void Build_prompt_teaches_checkpoint_commits_and_the_end_of_work_recompose()
    {
        ...
        prompt.Should().Contain(
            ...
            "that could silently resolve to nothing");
        prompt.Should().Contain(
            "stop here, do not reset",
            "an unresolved fork point aborts loudly instead of falling through to a bare reset");
    ...
    public void An_adopted_tasks_quoted_description_is_ruled_out_as_instruction()
    {
        ...
        prompt.Should().Contain("the title and quoted", "the rule covers the title, not only the fenced body")
            ...
            .And.Contain("rather than acting on it");
    }
    ...
    /// <summary>
    /// A human did register a session, but that session carried no display name
    /// (<c>RunDetails.RegisteredInteractiveSessionName</c> reads empty, not null) — a fact
    /// distinct from nobody having registered at all, and the prompt says so rather than
    /// collapsing the two into the same "nobody registered" text (Copilot review on PR #236).
    /// </summary>
    [Fact]
    public void Headless_build_prompt_distinguishes_a_blank_registered_address_from_no_registration()
    {
        TaskDetails task = SomeTask();
        task.InteractiveModeEnabled = true;

        string prompt = AgentPromptBuilder.Build(
            task, SomeProject(), "task/1-slug", _worktreePath, interactiveMilestoneAddress: "   ");

        prompt.Should().Contain("A human did register a session against this run");
        prompt.Should().Contain("nowhere to send to, not nobody to send to");
        prompt.Should().Contain("Log this once for the phase");
        prompt.Should().NotContain("Address:", "there is nothing resolved to address here");
        prompt.Should().NotContain("No registered human session is on record for this run right now");
    }
