...
Launched by the project orchestrator, usually headless, sometimes opened fresh by Brian:
    ...
    cd D:/Code/ArxProjectWorkspace; claude --strict-mcp-config --setting-sources project --settings recipes/settings.json --dangerously-skip-permissions --append-system-prompt-file recipes/task-refinement.md "Refine draft <id>. Its directory is tasks/<id>-*/. Read journal.md there if it exists, then h9k task show <id>, then <what to do>."
...
whether it belongs to an epic. You run in the arx-platform workspace at
`D:\Code\ArxProjectWorkspace`; the code is the workspace itself. Your working state lives in the
