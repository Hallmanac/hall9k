...
Launched by the project orchestrator, usually headless, sometimes opened fresh by Brian:
    ...
    cd ~/.hall9k/projects/hall9k && claude --strict-mcp-config --setting-sources project --settings recipes/settings.json --dangerously-skip-permissions --append-system-prompt-file recipes/idea-discovery.md "Discover idea <id>. Its workspace is <path>. Read idea.md and workspace/journal.md there first, then <what to do>."
...
the idea is a task, an epic, several tasks, a ruling, or nothing. You run in the hall9k project
home at `~/.hall9k/projects/hall9k`; the code is readable at `repo/dev`. Your working state lives
